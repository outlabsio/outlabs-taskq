...
- [ ] **S5-QD-C8-REVIEW-REQUEST · Direct retirement design gate** — assemble a targeted adversarial request that independently re-derives callers and executable contact surfaces, attacks the exact-ID read/IAM boundary, proves producer-before-consumer ordering and both rollback floors, checks shared-ledger/package-domain preservation, and binds R19-01 plus both seven-day windows. READY may open only C8-R1 caller migration; it cannot authorize producer removal, consumer removal, data/schema deletion, another lane, or Stage 6.
