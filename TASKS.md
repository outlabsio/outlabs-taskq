...
- [ ] **S5-RM-02B · Read-model facade, HTTP clients, parity, and B9** — Protocol 1.0.7’s generated `GET /queues/{queue}` and `GET /jobs?queue=&view=` identities now mount in the facade and both official clients. The dispatcher authenticates then queue-authorizes the query queue before cursor decoding; queue profile GET stays flat with ETag while conditional PUT returns only the canonical `{"profile": {...}}` envelope plus ETag. The shared models reject malformed `If-Match`, bind opaque cursor queue/view, and preserve the SQL `TQ001`/`TQ501` outcomes. Remaining: independent SQL/HTTP/raw-row parity and the separately controlled ready-only activation decision. `running` and `finished` remain structurally rejected by B9. Stop for targeted review before host adoption.
