...
| Contracts | Protocol v1 document revision 1.0.13 + Function Manifest/installed SQL 0.2.3 through immutable migration 0011 (+ ADR-012..030). ADR-029 freezes only finite running/finished queue pages and one exact workflow projection; ADR-030 preserves cancellation lock order through no-FK private counters. Capabilities remain at the exact 0010 set until proof-gated metadata migration 0012 |
| Next review | FR-02D-SQL is complete. Independent B9 evaluation now decides which of running, finished and workflow can enter metadata-only 0012; generated HTTP surface follows only for winners. No production deployment, QDarte migration, old-ledger import or C8 observation has occurred |
...
- [ ] **S5-QD-FR-02 · Native 0.2 prerequisite program** — from FR-01 evidence, freeze and deliver lossless follow-ups, dependencies/workflows, schedules, and only the finite operator projections QDarte actually needs. One capability per docs-first contract/migration/client/parity/race/plan/artifact slice, bridge-first at every SQL revision; QDarte may not emulate a missing capability.
  ...
  - [ ] **FR-02D · Finite operator projections** — independently contract, plan-test and activate only running/finished/workflow projections that meet their exact B9 and redaction gates; no timeline or generic reporting surface.
    ...
    - [ ] **FR-02D-ACTIVATE · Metadata-only winner activation** — B9 commit `988309c` proves all three candidates on both supported majors, and the Manifest freezes 0012's exact eight-capability post-state docs-first. The first activated page vector exposed 0011's committed PL/pgSQL composite-assignment defect; immutable 0013 is now specified docs-first to replace only that existing function body before activation acceptance.
    - [ ] **FR-02D-SURFACE · Generated clients/facade/parity**
    - [ ] **FR-02D-AUDIT · Dual-major and artifact evidence**
