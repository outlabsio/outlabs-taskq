...
| | |
|---|---|
| Stage | **Post-Stage-4 retirement eligibility + Stage-5 QDarte production pilot** — C8-R1 source is complete on three non-deploying branches: the bounded package caller/exact-ID floor, browser-local admin hint, combined enqueue+read identity, disabled-first compose inputs, and equality-checked depth/concurrency controls are pushed. Production remains in the accepted draining/paused/no-worker/no-gateway posture; disabled deployment and targeted acceptance are next. The next natural scheduler/object-store result remains mandatory audit evidence. The legacy-tools observation continues independently |
| Suite | taskq 505/505 regular with 1 opt-in skip on local PG18.3 using CI-shaped authenticated Redis, Ruff/format clean; QDarte runtime 1147/1147 + Ruff/MyPy 192; admin 113/113 + TypeScript/build; API taskq/contact slice 238/238 + Ruff/MyPy 193 on fresh scratch databases. A disposable PG18.3 production-shaped package proof applied 0001–0007, passed `verify()`, converged manifest v2 created→unchanged, and ended paused at `max_depth=1`, concurrency one, zero jobs/admissions |
...
| Next review | Deploy the exact C8-R1 commits disabled-first, issue/prove the combined caller identity, converge manifest v2 while paused, then execute the §5.4 health/drain/start/unpause/submission choreography only if the next natural backup succeeds; stop for targeted R1 acceptance before any direct producer becomes unreachable |
...
- [ ] **S5-QD-C8-R2 · Direct producer retirement and observation** — make historical and incumbent-host-taskq contact producers unreachable, retain direct consumers, rehearse the caller-floor+C7-API package rollback, and complete the seven-day/two-API-cycle zero-direct-change window before acceptance.

- [ ] **S5-QD-C8-R3 · Direct consumer retirement and observation** — after R2 acceptance, enforce server-side no-claim for the retired direct type, remove only contact-specific direct consumer/result/config surfaces, preserve every unrelated lane/shared ledger, and complete the seven-day/two-worker-cycle window plus paired zero-DML rollback.

- [ ] **S5-QD-C8-AUDIT · Direct contact retirement completion gate** — regenerate caller/source inventories, IAM, producer reachability, stale-worker claim negatives, exact direct/package/domain hashes, both windows, backups/restores, unrelated-lane positives, rollback rehearsals, and exact repository/deployment gates. Acceptance closes only the direct contact executable path; history/schema deletion, another lane, and Stage 6 remain closed.
