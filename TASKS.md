...
- [ ] **S5-QD-CV-04F · Controlled local contact canary** — API commits `7a887ba`, `08ddb0a`, `849d09f`, and `c5c171e` add one-day self-contained exact-scope harness tokens, a one-entity local-file keyed producer, a standalone harness entrypoint, and the Redis namespace required by the pinned QDarte auth runtime; workers commits `68c7e33` and `eb4f3c8` add a mandatory development-harness health assertion and the closed worker command. The isolated harness booted only on loopback with its dedicated non-superuser package DSN; the closed worker booted and its cross-action/wrong-queue checks returned 403 before the keyed pair. The first real keyed job (`created` then `existed`) reached the private reporter `inspect` operation, which returned 422 on both bounded attempts; the worker fenced both attempts as failed and no domain effect was committed. The facade and worker were stopped immediately. This is an implementation defect, not a contract question: diagnose the private inspect path and add a regression before another provider call. Direct queue and protected-ledger baseline digests remain the required oracle; no production, broad worker, retirement, or non-contact action is authorized.
