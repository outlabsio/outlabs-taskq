...
| Next review | FR-03B's machine effect inventory is green and pushed at QDarte runtime `6f18c71`: all 21 executable tasks match the closed handler/queue catalog, every operation has source evidence and a named owner, mutating operations require stable identity, and settlement-triggered domain mutation is executable-fail-closed. Exact native input/output and policy definitions are next |
...
- [ ] **S5-QD-FR-03 · Native QDarte registry and domain-effect layer** — relocate payload/result contracts out of the old queue API, register every active task once, generalize the ADR-022 reporter into a closed idempotent QDarte domain-effect protocol, and prove every handler under `taskq.testing` plus disposable real SQL/HTTP without starting the old worker.
  ...
  - [ ] **FR-03B · Exact native definitions and strict outputs** — define the 21 executable tasks once with exact queue, input/output identity, retry, lease, follow-up, effect and resource metadata; non-executable/historical declarations stay absent, and four independently derived sets must agree.
