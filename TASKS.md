...
| Suite | taskq 584/584 regular with 1 opt-in skip on local PostgreSQL 18.3 using the CI-shaped Redis; the previously accepted exact-16.14 lane is unchanged and Ruff is clean. Published `v0.1.0a7` remains the exact QDarte pin. Runtime is 1177/1177 with one unrelated dependency warning, clean Ruff and 196-file MyPy; workers are 706/706 with clean Ruff and 64-file MyPy; the API is 1776 passed/10 unchanged unrelated baseline failures with clean Ruff and 207-file MyPy. The native review family proves revision validation before egress, shared provider admission/settlement, database-time authoritative review/media/pipeline mutation, closed branch selection and identical response-loss replay without an old job/client/result route/completion hook. No service, provider, persistent database or production state changed |
