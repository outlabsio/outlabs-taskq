...
| Next review | CQ-11 is owner-approved and implemented docs-first in taskq `f9846a2`, runtime `c21960e`, API `d391e1a` and workers `db54852`. FR-03C-PHOTO remains open only for the real photo-specific hard-kill/reclaim conservation rehearsal and its raw provider/effect/artifact/follow-up oracle; do not start another family first |
...
- [ ] **S5-QD-FR-03 · Native QDarte registry and domain-effect layer** — relocate payload/result contracts out of the old queue API, register every active task once, generalize the ADR-022 reporter into a closed idempotent QDarte domain-effect protocol, and prove every handler under `taskq.testing` plus disposable real SQL/HTTP without starting the old worker.
  ...
  - [ ] **FR-03C · Native handler bindings** — refactor pure/read/effect cohorts to the taskq handler signature without constructing an old job/attempt/client, then prove every binding and follow-up graph through `taskq.testing`.
    ...
    - [ ] **FR-03C-PHOTO · Native photo-verification binding** — CQ-11 is resolved and the core binding is implemented across taskq `f9846a2`, runtime `c21960e`, API `d391e1a` and workers `db54852`. The source-derived inventory now records every remaining completion-hook handoff. Photo inputs carry exact per-entity retry/review plans with structural exhaustion, the handler inspects three mutually exclusive durable operations before provider work, uses shared ADR-031 reserve/settle control, applies photo-gate/retry-selection/terminal-blocker truth through the authoritative API, returns only preplanned native follow-ups, and reuses one job-scoped immutable two-vision-check artifact after response loss. Unit/full gates are green at taskq 584/1, runtime 1172, workers 681 and API 1764 pass/10 unrelated baseline failures. Keep this unchecked until a real process-kill after provider egress proves same-job reclaim, one effect choice, one final artifact/child set, and exactly one provider event or typed expired-unsettled generation without old rows.
