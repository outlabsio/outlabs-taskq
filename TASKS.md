...
- [ ] **S5-QD-P0 · QDarte isolated-dev preflight** — **blocked after safe partial evidence:** the guarded local PostgreSQL is 18.4 with `max_connections=100`, and the API currently uses the `postgres` superuser, confirming R11-01’s dedicated-facade-role requirement. The worker was started only with the pure `cluster_research_scope` allowlist and the no-network queue/claim/complete drill passed. Its cleanup recreated the normal broad allowlist, so the worker was immediately stopped rather than left able to claim unrelated local jobs. The full existing smoke remains red because unrelated `intake-worker` is absent; this pilot may not start it blindly because it can process non-pilot intake work. Source inspection also found `worker_job_events` is append-only with no `updated_at`; QP-09 now adds its `created_at` high-water plus a stable full-row digest for all six tables. Resume only after an explicit safe disposition for the unrelated intake worker/full-stack smoke and a current-source isolated-stack baseline; stop before P1 if the facade cannot have its own non-superuser DSN/pool or any isolation/privilege vector fails.
