...
| | |
|---|---|
| Stage | **Post-Stage-4 retirement eligibility + Stage-5 QDarte production pilot** — C8-R1 is locally cutover-ready on non-deploying branches. The bounded package caller/exact-ID floor, browser-local hint, combined enqueue+read identity, manifest controls, and one-command disposable route→facade→admission→closed-worker→status→rollback rehearsal are complete. Production remains in the accepted draining/paused/no-worker/no-gateway posture; no production action is required for the current local-readiness goal. The next natural scheduler/object-store result remains mandatory before any later enablement. The legacy-tools observation continues independently |
| Suite | taskq 505/505 regular with 1 opt-in skip on local PG18.3 using CI-shaped authenticated Redis, Ruff/format clean; QDarte runtime 1147/1147 + Ruff/MyPy 192; workers 628/628 + Ruff; admin 113/113 + TypeScript/build; API C8/contact slice 128/128 on a fresh named scratch database + Ruff/MyPy 193, plus the new cross-repo harness MyPy-clean. The disposable PG18.3 local cutover gate passed twice from fresh containers: 0001–0007 + `verify()`, non-superuser privilege negatives, disabled refusal, `created -> existed -> existed`, one planner/verifier/effect, one successful attempt, exact-ID terminal read, read-only admission denial, and paused zero-DML rollback with zero provider calls |
...
| Next review | Local engineering is complete. When production testing is explicitly resumed: deploy the exact C8-R1 commits disabled-first, issue/prove the combined caller identity, converge manifest v2 while paused, and execute the §5.4 bounded choreography only after a successful natural backup; stop for targeted R1 acceptance before any direct producer becomes unreachable |
