...
- [x] **S5-QD-CV-04C · Trusted-reporter pre-release** — version `0.1.0a5` is the immutable consumer artifact for ADR-022: it contains the trusted reporter kernel and no SQL-contract, migration, Protocol-v1, facade-route, or generated-client change. QDarte must exact-pin its published wheel hash before it configures the closed contact reporter/worker; no local package contact database migration or canary is implied by this release.
