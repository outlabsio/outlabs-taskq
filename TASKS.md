...
| | |
|---|---|
| Stage | **S4-03 restricted-runtime base complete; credential-log remediation gates the canary** — production API and worker now use the non-superuser runtime login, taskq is provisioned and enabled in legacy mode, and S4-03D must close the observed Redis-URI log leak before allowlisted work |
| Suite | 449/449 regular on PG18.3; 448/448 last run on PG16.14; 289/289 DB-free on Python 3.12 and 3.13; PG18 million-row plan gate 2/2; artifact matrix 12/12; deployed production host line 63/63 regular with 5 pre-existing opt-in skips; MyPy 62 files |
...
- [ ] S4-03D credential-log remediation: stop the upstream auth Redis client from logging credential-bearing connection URIs, rotate the exposed Redis credential through both production resources, and prove startup/connection logs plus representations contain no old or new secret before canary traffic
