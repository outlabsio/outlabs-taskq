...
| Next review | Immutable migration 0009 may now implement the frozen workflow/dependency SQL kernel against the shipped 0.2.1 bridge. It must preserve the exact capability set, fresh/full upgrade chains, privilege equality, graph races and bounded plans before typed HTTP/client work. No production deployment, QDarte migration, old-ledger import or C8 observation work is part of the active goal |
