...
- [ ] **S5-QD-FR-03 · Native QDarte registry and domain-effect layer** — relocate payload/result contracts out of the old queue API, register every active task once, generalize the ADR-022 reporter into a closed idempotent QDarte domain-effect protocol, and prove every handler under `taskq.testing` plus disposable real SQL/HTTP without starting the old worker.
  ...
  - [ ] **FR-03C · Native handler bindings** — refactor pure/read/effect cohorts to the taskq handler signature without constructing an old job/attempt/client, then prove every binding and follow-up graph through `taskq.testing`.
    ...
    - [x] **FR-03C-DISCOVERY-IMPORT-ATOMICITY · Import services cannot escape the effect transaction** — runtime `3d2401d` exposed caller-owned import/normalization transaction mode and API `dbba509` now uses it for the already-bound discovery import effect. Import batches and normalization flush for same-session visibility but cannot commit before the native effect receipt; the legacy service-owned mode remains the default. Seven focused API vectors, 1189 runtime tests, Ruff and source MyPy are green. No route, worker, artifact, database, provider or production state changed.
...
**Adjudication (2026-07-24): approved as recommended through owner-authorized
internal review while the external reviewer is unavailable.** Source
re-derivation of `scripts/deploy.ts` confirms that the existing deployment
pipeline already owns media preparation/derivation, frontend export, build,
deploy and route verification. The frozen split therefore uses one
producer-planned `frontend_deploy_scope` child rather than inventing another
host-authority family. Publication records database truth and pending
derivative posture only. The external operation uses one create-once
job-scoped state file, recovers completed work without rerun and treats a
started-but-nonterminal state as `execution_unknown`; it never automatically
duplicates an ambiguous deployment.
