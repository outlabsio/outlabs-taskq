...
| Suite | taskq 577/577 regular with 1 opt-in skip on local PostgreSQL 18.3 after the schedule facade/client/runtime/fake slice; Ruff/format are clean. The immutable 0010 SQL baseline remains 560/560 plus one skip and the expanded million-row claim/dependency/finalizer/schedule plan gate remains 2/2 on both PostgreSQL 18.3 and exact 16.14. FR-02C-AUDIT owns the exact-0.2.2 dual-major repeat, DB-free lane and complete Python 3.12/3.13 × core/HTTP/OutLabs artifact matrix. FR-01 repository-local drift gates pass across all four QDarte repositories; runtime is 1151/1151, workers 629/629, and admin 114/114 plus TypeScript/build. The API inventory gate passes independently; its unrelated whole-repository baseline currently has 15 order/environment failures among 1738 tests and remains a required cleanup before FR-AUDIT. The disposable PG18.3 local cutover gate passed twice from fresh containers through 0007; QDarte remains pinned to immutable a6 until the native-orchestration release is deliberately cut |
| Contracts | Protocol v1 document revision 1.0.12 + Function Manifest/installed SQL 0.2.2 through immutable migrations 0001–0010 (+ ADR-012..028). `followups`, `dependencies_workflows`, `read_model_list_ready`, `admission_reservations`, and `schedules` are active by exact metadata. The schedule operator facade/clients and runtime loop remain construction-gated by exact 0.2.2 metadata plus the `schedules` capability; ADR-028 excludes the seeded janitor identity from all HTTP routes. ADR-018 locks operator UI stack (React/Vite/TanStack/Base UI) |
| Next review | FR-02C-SURFACE is complete. FR-02C-AUDIT now repeats the dual-major, DST/catch-up/race/plan/resource and complete installed-artifact evidence before any QDarte consumer or package release. No production deployment, QDarte migration, old-ledger import or C8 observation has occurred |
