...
| | |
|---|---|
| Stage | **Stage 5 QDarte full replacement** — the owner has retired the contact-only strangler direction as the destination. The only active goal is one native taskq system for every QDarte lane, followed by deletion of both old queue implementations, every compatibility mode/wrapper, and their execution data. Business content remains; queue history is not migrated. FR-00/01/02 are complete and FR-03 is active. The native contact, website, classification, photo, editorial, listing-research, content-synthesis, translation and review effect families are complete. Production remains untouched |
| Suite | taskq 584/584 regular with 1 opt-in skip on local PostgreSQL 18.3 using the CI-shaped Redis; the previously accepted exact-16.14 lane is unchanged and Ruff is clean. Published `v0.1.0a7` remains the exact QDarte pin. Runtime is 1180/1180 with one unrelated dependency warning, clean Ruff/changed-file format and 196-file MyPy; workers remain 710/710 with clean Ruff and 64-file MyPy; the API remains 1779 passed/10 unchanged unrelated baseline failures with clean Ruff and 208-file MyPy. Runtime commits `c261749`/`712d4f8` add the exact grounded/search/proxy control catalog, reject caller authority, make buzz artifact-only and replace recursive rescue payloads with one finite discriminated work unit plus only an exact photo branch. No service, provider, persistent database or production state changed |
...
- [ ] **S5-QD-FR-03 · Native QDarte registry and domain-effect layer** — relocate payload/result contracts out of the old queue API, register every active task once, generalize the ADR-022 reporter into a closed idempotent QDarte domain-effect protocol, and prove every handler under `taskq.testing` plus disposable real SQL/HTTP without starting the old worker.
  ...
  - [ ] **FR-03D · General idempotent domain effects** — replace the 12 old result-route families with one closed inspect/apply reporter union, authoritative plan validation and stable job/family/entity/operation idempotency.
    ...
    - [ ] **FR-03D-PHOTO-FIND · Native photo-find family** — runtime commit `de3720c` freezes the executable boundary: exact entity-covering mutually exclusive verify/review plans, scope and entity equality, immutable relative candidate artifacts, bounded selection manifests, the closed `photo_application/apply` effect and typed replay response. API commit `a91ab9b` adds the private reporter binding plus a queue-independent mutation kernel: stored-input entity authority, server-rooted path containment, manifest/result equality, SHA-256 checks before mutation, PostgreSQL-time domain application and stable replay receipts. Runtime passes 1183; the focused API reporter/HTTP/artifact suite passes 32 with Ruff, format and MyPy clean. The full API run reaches 1803 passes with the same unrelated route/environment/inventory fixture failures carried by this development line. Next: bind the provider-injected worker and prove effect/artifact/child conservation without old job, attempt, client, result route or completion hook.
