...
- [ ] **S5-QD-HOST-GATE-01 · QDarte fresh migration baseline** — P1's isolated package pin/configuration/unit tests are drafted in fresh staging worktrees, but the current QDarte fresh-database chain is independently red before those host tests run: migration `20260715_0070_host_native_worker_lanes` rejects nonzero desired replicas while earlier migrations seed `media=1`. Resolve that incumbent QDarte migration-chain defect through its own owner-reviewed immutable-migration posture, then prove a fresh dedicated host test database reaches head before resuming P1. This is not a package migration, does not authorize `qdarte_pilot_dev`, and must not change the incumbent direct queue as a pilot side effect.

- [ ] **S5-QD-P1 · QDarte disabled host boundary** — blocked by S5-QD-HOST-GATE-01. Once the current QDarte fresh migration baseline is green, add the exact a3 pin, disabled-by-default package-facade/runtime settings, and the dedicated non-superuser facade DSN/pool targeting only `qdarte_pilot_dev`. Preserve the existing direct contact-verify queue and copied `/ops/taskq`/`/worker/taskq` surface untouched and do not reuse it for the pilot. Prove disabled boot makes no contact with `qdarte_pilot_dev`; the distinct pilot worker's lifecycle-fixed allowlist remains exactly `qdarte.cluster_research.pilot`. Stop before P2 database creation/provisioning, any public producer, or any non-pilot worker.
