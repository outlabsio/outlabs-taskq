...
| Next review | FR-01 must produce an executable source/deletion manifest and native-capability gap matrix. Contract-visible gaps then enter docs-first FR-02 slices. No production deployment, old-ledger migration, or C8 observation work is part of the active goal |
...
- [ ] **S5-QD-FR-01 · Executable inventory and deletion manifest** — generate checked-in closed manifests for every active/dormant task, handler, producer, queue lifecycle call, result/effect path, follow-up edge, dependency, schedule, route, model, table, function, setting, process and admin consumer across the four QDarte repositories. Classify every relation as domain-retain/domain-relocate/queue-delete/historical-migration-only, validate the queue/cohort map, and make CI fail on unclassified queue symbols or registry/handler drift. Stop and record any behavior the accepted taskq contracts cannot produce.
