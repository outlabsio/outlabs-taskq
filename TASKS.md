...
| | |
|---|---|
| Stage | **Stage 5 QDarte full replacement** — the owner has retired the contact-only strangler direction as the destination. The only active goal is one native taskq system for every QDarte lane, followed by deletion of both old queue implementations, every compatibility mode/wrapper, and their execution data. Business content remains; queue history is not migrated. FR-00/01/02 are complete: every native orchestration prerequisite is now contracted, implemented and accepted. FR-03 is active; CQ-11 is resolved and the photo-verification contract/core binding is implemented, with its real process-kill conservation rehearsal still gating FR-03C-PHOTO acceptance. Production remains untouched |
| Suite | taskq 584/584 regular with 1 opt-in skip on local PostgreSQL 18.3 using the CI-shaped Redis; the previously accepted exact-16.14 lane is unchanged and Ruff is clean. Published `v0.1.0a7` remains the exact QDarte pin. Runtime is 1172/1172 with clean Ruff/196-file MyPy; workers are 681/681 with clean Ruff/63-file MyPy; the API is 1764 passed/10 unrelated baseline failures after removing the two new inventory-oracle failures, with clean Ruff/201-file MyPy. The photo slice adds a source-backed completion-hook sweep, strict structural retry plans, one mutually exclusive effect choice, shared provider control, two source-equivalent vision checks, and a replayable job-scoped immutable artifact. The earlier classification-only hard-kill evidence remains valid but does not substitute for the pending photo-specific rehearsal. No old worker, persistent QDarte database or production state changed |
...
| Next review | CQ-11 is owner-approved and implemented docs-first in taskq `f9846a2`, runtime `c21960e` plus validation follow-up `e53b12b`, API `d391e1a`, and workers `db54852` plus replay-proof follow-up `800445c`. FR-03C-PHOTO remains open only for the real photo-specific hard-kill/reclaim conservation rehearsal and its raw provider/effect/artifact/follow-up oracle; do not start another family first |
...
- [ ] **S5-QD-FR-03 · Native QDarte registry and domain-effect layer** — relocate payload/result contracts out of the old queue API, register every active task once, generalize the ADR-022 reporter into a closed idempotent QDarte domain-effect protocol, and prove every handler under `taskq.testing` plus disposable real SQL/HTTP without starting the old worker.
  ...
  - [ ] **FR-03C · Native handler bindings** — refactor pure/read/effect cohorts to the taskq handler signature without constructing an old job/attempt/client, then prove every binding and follow-up graph through `taskq.testing`.
    ...
    - [ ] **FR-03C-PHOTO · Native photo-verification binding** — CQ-11 is resolved and the core binding is implemented across taskq `f9846a2`, runtime `c21960e`/`e53b12b`, API `d391e1a` and workers `db54852`/`800445c`. The source-derived inventory now records every remaining completion-hook handoff. Photo inputs carry exact per-entity retry/review plans with structural exhaustion and reject duplicate entity identities; the handler inspects three mutually exclusive durable operations before provider work, uses shared ADR-031 reserve/settle control, applies photo-gate/retry-selection/terminal-blocker truth through the authoritative API, returns only preplanned native follow-ups, and reuses one job-scoped immutable two-vision-check artifact after response loss. Replay is pinned to the same durable job identity. Unit/full gates are green at taskq 584/1, runtime 1172, workers 681 and API 1764 pass/10 unrelated baseline failures. Keep this unchecked until a real process-kill after provider egress proves same-job reclaim, one effect choice, one final artifact/child set, and exactly one provider event or typed expired-unsettled generation without old rows.
