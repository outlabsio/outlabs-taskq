...
| Next review | FR-02 is complete and QDarte may consume native 0.2 locally. FR-03 must relocate final payload/result contracts into one native registry, prove every handler under the native test/runtime surfaces, and generalize the trusted domain-effect protocol without starting either old worker; no production deployment, old-ledger import or C8 observation has occurred |
...
- [ ] **S5-QD-FR-03 · Native QDarte registry and domain-effect layer** — relocate payload/result contracts out of the old queue API, register every active task once, generalize the ADR-022 reporter into a closed idempotent QDarte domain-effect protocol, and prove every handler under `taskq.testing` plus disposable real SQL/HTTP without starting the old worker.
  - [ ] **FR-03-RELEASE · Immutable native-orchestration alpha** — prepare, independently install-smoke, tag and publish `0.1.0a7` from the CI-green FR-02 completion tree before any QDarte repository pins or consumes SQL 0.2.3. The release must contain exactly migrations 0001–0013, the 65-function machine catalog and Protocol 1.0.13 generated surface; the published wheel hash and source identity are recorded before FR-03 source work.
