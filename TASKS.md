...
- [ ] **S5-QD-C7-R16-01-03 · Environment-plan delta** — replace the invalid cohort with a valid containing scope plus exact one-item `place_ids` and `limit=1`; freeze a runtime-owned verifier behind a network-enforced, bounded dual-homed egress proxy; and move the facade/worker to an unpublished private Compose network with exact service identity and readiness-bearing health. Update all tables, stop conditions, sequences, and review oracles consistently, then stop for a targeted delta check before C7-01.
