...
| Next review | The 0009 SQL kernel and direct typed transport are complete locally. FR-02B-SURFACE now owns capability-gated HTTP routes, both generated clients, the high-level facade, substitutable fake, SQL/HTTP/fake parity and authorization-before-decode evidence; no production deployment, QDarte migration, old-ledger import or C8 observation is part of the active goal |
...
- [ ] **S5-QD-FR-02 · Native 0.2 prerequisite program** — from FR-01 evidence, freeze and deliver lossless follow-ups, dependencies/workflows, schedules, and only the finite operator projections QDarte actually needs. One capability per docs-first contract/migration/client/parity/race/plan/artifact slice, bridge-first at every SQL revision; QDarte may not emulate a missing capability.
  ...
  - [ ] **FR-02B · Dependencies and workflows** — freeze and ship immutable migration 0009 plus producer-safe workflow identity, atomic edge admission, promotion/cascade/finalization, bounded reads and concurrency evidence.
    ...
    - [ ] **FR-02B-SURFACE · Generated workflow surface and parity** — add capability-gated facade routes, sync/async clients, high-level APIs and the testing fake from the one Protocol catalog; prove all-declared-queue authorization before body/dependency decode, strict projections, SQL/HTTP/fake raw-state parity, response-loss replay and absence below 0.2.1.
    - [ ] **FR-02B-AUDIT · Workflow completion evidence** — exact-tip CI, dual-PG full chains/plans/races/resources, Python 3.12/3.13 wheel/sdist × core/HTTP/OutLabs isolation and an independent-oracle audit before FR-02C.
