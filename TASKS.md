...
- [ ] **S5-QD-C7-02-REVIEW-REQUEST · One-place cohort targeted gate** — pin the C7-02 cross-repo source/artifact identities and evidence packet; independently re-derive the one-place bound, keyed admission identity, taskq attempts/events, direct full-row conservation, stable effect, usage and egress counts, least-privilege topology, disk cleanup scope, disclosed fail-closed incidents, and final paused/draining posture. Because the external reviewer is unavailable, the response must be explicitly owner-authorized internal/non-independent. READY may open only C7-03; no direct retirement, another lane, or Stage 6.
