...
| | |
|---|---|
| Stage | **Post-Stage-4 retirement eligibility + Stage-5 QDarte production pilot** — internal Round 19 accepts the complete C7 contact-lane evidence; a docs-first direct-retirement specification is next. The legacy-tools observation continues independently |
...
| Next review | The direct contact retirement specification must stop for its own targeted review before any producer/consumer removal; targeted L1 acceptance separately gates legacy-tools L2 |
...
- [ ] **S5-QD-C8-SPEC · Freeze direct contact producer/consumer retirement** — design the smallest docs-first retirement sequence now that C7 is accepted. It must source-audit every direct contact caller/consumer and shared-ledger dependency; preserve all unrelated `qdarte_ops` rows, tables, migrations, workers, and rollback images; define producer removal before consumer removal across separate observation windows; keep package history and the paused/draining rollback baseline; bind R19-01 by recording the next scheduled 03:15 backup before any destructive implementation; and require exact direct/package/domain high-water oracles plus a zero-DML rollback. Stop for targeted review before changing source, configuration, services, databases, or retirement state. Another lane and Stage 6 remain closed.
