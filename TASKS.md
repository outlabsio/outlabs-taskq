...
| Suite | taskq 584/584 regular with 1 opt-in skip on local PostgreSQL 18.3 after the CQ-13 completion, with the previously accepted identical exact-16.14 lane unchanged; Ruff is clean. Published `v0.1.0a7` remains the exact QDarte pin. Runtime is 1170/1170 with clean Ruff/196-file MyPy; workers are 676/676 with clean Ruff/61-file MyPy; the API is 1761 passed/10 unrelated baseline failures with clean Ruff/201-file MyPy. The closed inventory is 302 files/23 declarations/21 handlers/30 relations/130 routes. A self-cleaning classification-only hard-kill rehearsal proves real SQL/HTTP execution, actual `SIGKILL` after provider return, lease-expired same-job reclaim, immutable unknown-cost generation rollover, one final domain effect, and exact conservation `2 provider calls = 1 durable provider event + 1 expired_unsettled generation`, with zero old job/event rows. No old worker, persistent QDarte database or production state changed |
...
| Next review | The native classification family and CQ-13 hard-kill gate are complete on non-deploying branches. S5-QD-FR-CQ-11 is now the only blocker to FR-03C-PHOTO; its docs-first reclassification of photo verification must be owner-adjudicated before that binding starts |
