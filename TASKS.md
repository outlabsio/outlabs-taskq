...
- [ ] **S5-QD-P3 · QDarte deterministic pilot adapter** — register only the isolated `qdarte.cluster_research.pilot` task type and adapt the frozen synthetic `cluster_research_scope` input to its canonical bounded result. Prove its digest equals the existing pure computation and that it has no I/O, followup, legacy enqueue, QDarte domain write, or shared-registry/closed-literal mutation. It may add an internal local harness only; no public producer or worker start is authorized.
