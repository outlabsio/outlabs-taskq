...
- [x] **S5-RM-HOST-A3 · Route-deferred bridge artifact** — immutable release branch based exactly on `40aa9b5`, with only package-version metadata changed to `0.1.0a3`. It accepts the closed SQL-contract set through 0.1.4 while its reserved H-08/H-11 commands remain deferred; release evidence must prove typed TQ501, OpenAPI absence, and no generated client method before host pin/deployment. This branch does not contain migration 0006 and is deliberately the inactive-ready rollback bridge, not the route-exposure artifact.

- [ ] **S5-RM-02B · Read-model facade, HTTP clients, parity, and B9** — migration `0005_read_model_conformance.sql` now makes SQL contract 0.1.4 real: direct SQL has the same unknown→`TQ001`, existing-inactive→`TQ501`, and existing-empty→page dispositions as the future HTTP surface; `verify()`, catalog parity, fresh install, and full upgrade vectors cover the immutable chain. ADR-020's bridge accepts `0.1.2|0.1.3|0.1.4` while preserving the pre-bridge rejection proof; no capability is activated. Next generate the two HTTP read commands and conditional profile update from the Protocol 1.0.7 table; preserve authenticate → queue-authorize → cursor parsing; add independent SQL/HTTP/raw-row parity and redaction/cursor/conflict oracles, then make the separately controlled ready-only activation decision. `running` and `finished` remain structurally rejected by B9. Stop for targeted review before host adoption.
