...
- [ ] **S5-QD-CONSOLIDATION-REVIEW · Targeted direct-queue decision review** — Round 12 must independently reconstruct the direct catalog/routes/worker/result path, challenge the separate-database and mode-exclusivity claims, and attack side-effect idempotency, retry/reclaim, hard-kill, credentials, rollback, and production evidence. It may authorize only a later implementation specification; it authorizes no current QDarte change.
