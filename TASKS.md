...
- [ ] **S5-QD-P0 · QDarte isolated-dev preflight** — reproduce the existing isolated stack and no-network worker smoke, then record actual source identities, compose mounts/guards, PostgreSQL version/ceiling, API/facade role attributes, connection arithmetic, and the six-table legacy-drift baseline. Stop before P1 if the facade cannot have its own non-superuser DSN/pool or any isolation/privilege vector fails.
