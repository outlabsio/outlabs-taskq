...
| | |
|---|---|
| Stage | **Post-Stage-4 retirement eligibility + Stage-5 QDarte C7 cohort** — C7-01's disabled Mini87 production preflight is internally accepted; the frozen one-place C7-02 cohort is the next active QDarte slice. The legacy-tools observation continues independently |
| Suite | taskq 505/505 regular with 1 opt-in skip on local PG18.3 using CI-shaped authenticated Redis, Ruff/format clean; QDarte runtime 1144/1144 + MyPy 194; workers 627/627 + MyPy 53; API C7 boundary 34/34 + Ruff/format/MyPy clean (broad local baseline: 1553 passed, 155 environment skips, 8 unrelated failures) |
...
- [ ] **S5-QD-C7-02 · One-place production cohort** — execute only the frozen `country` scope plus one exact `place_ids` entry and `limit=1` under the accepted source/image/database identities. Capture fresh direct/effect/package baselines; enter package mode through same-process drain; start only the bounded egress gateway and closed worker; unpause only after direct publication is disabled; reuse one pre-recorded idempotency key for `created` then `existed`; require the exact selected place, one terminal job, one stable effect, and reconciled independent egress/usage/raw-ledger counters; stop worker/egress and record the selected zero-DML rollback posture. Any ambiguity, direct insert, planner mismatch, provider/counter mismatch, secret/topology drift, disk insufficiency, or second place stops the cohort. Stop for targeted acceptance before C7-03, retirement, another lane, or Stage 6.
