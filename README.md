...
**Status:** alpha — implementation has reached Protocol v1 document revision 1.0.13 / installed SQL contract 0.2.3 / [ADR-001..030](docs/adr/README.md). **Stages 1 through 4 and the complete native-orchestration prerequisite program are accepted; the full QDarte legacy replacement is the current program** — the SQL kernel, typed client, worker/CLI, consumer testing helpers, generated HTTP clients, mounted FastAPI facade, authorization boundary, long-poll hub, composable runtime, OutLabs authorization tools, durable admission, atomic follow-ups, sealed workflows/dependencies, database-time schedules, and finite ready/running/finished/workflow projections are implemented. See the live [`TASKS.md`](TASKS.md) board for current counts and work.
...
- `outlabsAPI` (first production host — embedded topology)
