...
| Queues | `queue list`, `health`, `show`, `ensure`, `update`, `pause`, `resume`, `purge`, `redrive-failed`; flow control: `set-breaker`, `set-breaker-rate`, `trip-breaker`, `close-breaker`, `set-aging`, `set-flow-limit` |
