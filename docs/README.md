...
[ADR-001..022](./adr/README.md). Accepted 2026-07-18 across the design reviews, contract adjudications, and the Stage-2/Stage-3 gates; ADR-018 (operator UI stack), ADR-020 (SQL-contract compatibility), ADR-021 (read-model conformance repairs), and ADR-022 (trusted worker side-effect reporting) are accepted. Reopening one requires new evidence and a new ADR.
...
| [Stage 5 QDarte Contact Verify Compatibility and Cutover Specification](./Task%20Queue%20Stage%205%20QDarte%20Contact%20Verify%20Compatibility%20and%20Cutover%20Specification.md) | C6 local caller compatibility, one-publisher mode, direct-drain, and rollback evidence, followed by the separately gated C7 production-evidence sequence | Planning frozen only. C6-00 may inventory the current direct lane; no cutover, lasting package database, production action, or retirement is authorized |
