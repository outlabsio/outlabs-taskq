...
| [Stage 5 QDarte Contact Verify Compatibility and Cutover Specification](./Task%20Queue%20Stage%205%20QDarte%20Contact%20Verify%20Compatibility%20and%20Cutover%20Specification.md) | C6 local caller compatibility, one-publisher mode, direct-drain, and rollback evidence, followed by the separately gated C7 production-evidence sequence | C6-03A exposed the replay-planning gap; ADR-023's general admission primitive must pass S5-AR-AUDIT before isolated C6 resumes. No production action or retirement is authorized |
