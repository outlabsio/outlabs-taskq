...
| [Stage 5 QDarte Contact Verify Compatibility and Cutover Specification](./Task%20Queue%20Stage%205%20QDarte%20Contact%20Verify%20Compatibility%20and%20Cutover%20Specification.md) | C6 local caller compatibility, one-publisher mode, direct-drain, and rollback evidence, followed by the separately gated C7 production-evidence sequence | C6-01..04 local evidence is complete through exact-hash durable admission and all three zero-row-copy rollback postures; C6-AUDIT now gates C7-00 planning. No production action or retirement is authorized |
