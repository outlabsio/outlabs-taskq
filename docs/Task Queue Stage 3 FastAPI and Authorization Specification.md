...
Borrowed HTTPX clients are never closed or globally reconfigured. The TaskQ timeout is nevertheless
passed on every TaskQ request, so a borrowed client's unrelated default timeout cannot silently
replace the TaskQ contract. Sync/async clients do not share a mutable session across process forks.
After the command-specific retry policy is exhausted, both clients normalize HTTPX transport
exceptions to retryable `TaskqUnavailableError` with the original exception retained only as the
local cause. Raw HTTPX exception classes never cross the TaskQ transport boundary.
