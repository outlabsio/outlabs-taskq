...
> **Status:** CANONICAL for SQL contract 0.2.4 — 2026-07-25. Closes R2-08 and incorporates ADR-012/013/019/021/023/024/026/027/029/030/033: every function the contract ships is listed here with identity, grants, raises, and an executable body (or a pointer to its normative body in the Unified Spec §5/§11, Durable Admission Reservation Specification, or Native Orchestration Specification as amended here). Migrations 0001 through 0015 derive from THIS document; `verify()` compares the live catalog against this manifest (ADR-011 §4). A function not listed here does not exist in 0.2.4 — no success-returning stubs.
...
manifest-declared finite set when PostgreSQL dump/restore proves equivalent
parse presentation. SQL contract 0.2.4 has exactly one alternate:
`schedules` may use canonical digest
`ebdc41f67bf31c91d1fe215f9c59304f` or restored digest
`7c66926dfa44623d66e15c99a07b1665`; the only difference is associative
parenthesization of `schedules_name_ck`. Constraint count, identities and
types remain exact. No other alternate is accepted. PostgreSQL 16/18
dump/restore plus a deliberate name-bound mutation are required evidence.
...
## 19. Contract 0.2.4 — ADR-033 bounded worker presence (2026-07-25)

Migration `0014_worker_presence_projection.sql` requires exact contract 0.2.3
and the exact post-0012 capability set. It installs bounded inactive backing,
tightens the existing heartbeat's accepted label domain without changing its
identity, and sets contract 0.2.4. Metadata-only migration
`0015_activate_worker_presence.sql` requires exact 0.2.4 plus the complete
0014 catalog and activates only `worker_presence` after the recorded
PostgreSQL 16/18 B9 proof.

1. **Composite types.** Migration 0014 adds exactly:

   ```text
   taskq.worker_presence_item AS (
     worker_id text, declared_queues text[], version text,
     started_at timestamptz, last_seen_at timestamptz,
     online boolean, running_jobs bigint, shutdown_requested boolean
   )

   taskq.worker_presence_page AS (
     as_of timestamptz, items taskq.worker_presence_item[],
     next_last_seen_at timestamptz, next_worker_id text
   )
   ```

   H-02 applies. The item contains no hostname, pid, arbitrary metadata,
   credential/network detail, raw shutdown timestamp, job/attempt identity,
   fence, token or diagnostic data.
...
2. **Public identity.**
   ...
   The function is `STABLE SECURITY DEFINER`, owner-owned, path-pinned and
   PUBLIC-revoked. It validates limit `1..100` and an all-null or all-present
   cursor pair, checks capability `worker_presence`, stamps database `as_of`,
   and reads at most `limit + 1` workers ordered by
   `(last_seen_at DESC, worker_id DESC)`. `online` is true only when
   `last_seen_at > as_of - interval '180 seconds'`.
   `shutdown_requested` is derived only from whether the private timestamp is
   non-null. Running counts use only live `running` jobs.

   The normative body is:
   ...
   BEGIN
       IF p_limit IS NULL OR p_limit NOT BETWEEN 1 AND 100
          ...
          OR (p_after_worker_id IS NOT NULL
              AND (p_after_worker_id = '' OR length(p_after_worker_id) > 200)) THEN
       ...
       SELECT ARRAY(
           SELECT ROW(
               w.worker_id, w.queues, w.version, w.started_at, w.last_seen_at,
   ...
   A direct SQL client must not get a wider projection merely because it
   bypasses HTTP.

3. **Heartbeat validation tightening.** The existing identity remains:
   ...
   Before its existing upsert, it rejects unless `worker_id` is 1–200
   characters, `p_queues` contains 1–32 distinct values and every value
   matches `^[a-z0-9_]{1,57}$`, and `p_version` is null or at most 200
   characters. More than 32 queues, a duplicate, invalid queue, or an
   over-200-character version is `TQ422`. Hostname, pid and meta remain private
   heartbeat-only fields. Migration 0014 refuses to install if an existing
   worker row violates the newly exact public domain; it never silently
   rewrites presence.

4. **Indexes.** Migration 0014 adds exactly:

   ```text
   workers_presence_page_idx
     ON taskq.workers(last_seen_at DESC, worker_id DESC)
   taskq_jobs_worker_running_idx
     ON taskq.jobs(worker_id) WHERE status = 'running'
   ```

   Both are contract backing. Their existence is not performance proof; B9
   must demonstrate the bounded page on PostgreSQL 16 and 18 with one million
   presence rows.

5. **Live page semantics.** Each function call is one statement-consistent
   live page, not a historical snapshot. A heartbeat between pages can move a
   worker ahead of the prior cursor and omit it from the ongoing sweep.
   Fleets of at most 100 fit in one page. Every instant and `online` derive
   from database time only.

6. **Metadata and compatibility.** ADR-020's supported set becomes exactly
   `{0.1.2,0.1.3,0.1.4,0.1.5,0.2.0,0.2.1,0.2.2,0.2.3,0.2.4}` before
   migration 0014. Migration 0014 sets version 0.2.4 while retaining exactly:

   ```json
   {"active":["admission_reservations","dependencies_workflows","followups","read_model_list_finished","read_model_list_ready","read_model_list_running","read_model_workflow","schedules"]}
   ```

   After the B9 evidence, migration 0015 replaces metadata by exact equality:

   ```json
   {"active":["admission_reservations","dependencies_workflows","followups","read_model_list_finished","read_model_list_ready","read_model_list_running","read_model_workflow","schedules","worker_presence"]}
   ```

   No other capability is implied. Activation and deactivation are immutable
   metadata migrations, never manual DML.

7. **Evidence.** `verify()` and an independent oracle assert exact
   composites/function/indexes/grants/metadata and the unchanged raw-view
   posture. Fresh/full 0001→0015 chains on PostgreSQL 16/18 prove inactive
   `TQ501` at 0014 and success at 0015, all heartbeat bounds, ordering,
   pagination, live cross-page omission, redaction, global authorization
   before cursor decode, SQL/HTTP/fake/raw-row parity, bounded plans, resource
   cleanup and installed artifacts.

## 20. Contract 0.2.5 — ADR-035 native workflow-member continuations (2026-07-28)

Definition migration `0016_workflow_continuations.sql` requires exact contract
0.2.4, migrations 0001–0015 and the active post-0015 capability set. It
installs the compatibility bridge below, sets contract 0.2.5, and leaves
`workflow_continuations` absent. Metadata-only migration
`0017_activate_workflow_continuations.sql` requires exact 0.2.5, clean catalog
and the exact inactive capability set. WFC-I05 records the completed consumer
drift, dual-major race/B9, parity, auth, restore, rollback and artifact
evidence; the migration activates only that capability.

1. **Catalog additions.**

   ```text
   taskq.workflows append:
     member_limit integer NULL CHECK (member_limit BETWEEN 1 AND 1000000)
     continuation_policy_hash text NULL
       CHECK (continuation_policy_hash ~ '^[0-9a-f]{64}$')
     CHECK (continuation_policy_hash IS NULL OR member_limit IS NOT NULL)

   taskq.workflow_member_counts append:
     admitted_total bigint NULL CHECK (admitted_total >= 0)
   ...
   taskq.claimed_job append:
     continuation_policy_hash text

   taskq.workflow_profile append:
     member_limit integer, admitted_total bigint,
     remaining_capacity bigint, continuation_policy_hash text
   ```

   Pre-0016 workflows retain null limit, admitted total and policy and cannot
   receive memberized continuations. New workflows always have a non-null
   limit; a new null-policy workflow is budgeted but detached-only.

2. **New public overloads.**

   ```text
   taskq.create_workflow(text,text,jsonb,text[],text,integer,text)
     arguments:
       p_workflow_key, p_kind, p_params, p_declared_queues, p_actor,
       p_member_limit, p_continuation_policy_hash
     RETURNS taskq.workflow_result
     EXEC taskq_producer
     raises TQ001, TQ409, TQ422, TQ501

   taskq.claim_jobs(text,text,integer,text[],integer,text,uuid,text[])
     final argument p_continuation_policy_hashes
     RETURNS taskq.claim_batch
     EXEC taskq_runner
     raises TQ422, TQ501, TQ503
   ...
   `pg_catalog,taskq,pg_temp`, PUBLIC-revoked and granted only as named.
   The new create and complete overloads require every argument explicitly at
   SQL level; Python/wire models own ergonomic defaults. This prevents
   ambiguous resolution with existing identities.

3. **Compatibility identities.**

   - Existing five-argument `create_workflow` creates a new null-policy
     workflow at limit 10,000. Replay of a pre-contract row uses its original
     identity comparison and does not invent capacity.
   - Existing seven-argument `claim_jobs` filters
     `continuation_policy_hash IS NULL`, including targeted claim.
   - Existing six-argument `complete_job` may settle only null-policy parents
     and may emit only detached follow-ups.
   - Existing enqueue/workflow identities remain, but their bodies reserve
     lifetime capacity for new-contract workflows and stamp the workflow policy
     on admitted jobs.

4. **Follow-up JSON extension.** Each follow-up object retains every 0.2.0
   field and may append `"workflow_member": true`. Null/false are rejected when
   the member field is present; omission is detached. The complete policy
   witness is a separate overload argument and is never handler output.

5. **Private helpers.** Migration 0016 replaces the body, but not the identity,
   of `taskq._enqueue_followup(uuid,text,jsonb,integer)` and adds exactly:

   ```text
   taskq._reserve_workflow_members(uuid,integer,text)
     arguments:
       p_workflow_id, p_count, p_continuation_policy_hash
     RETURNS bigint
   ```

   The return is exact remaining lifetime capacity, or null for a legacy
   workflow. Both helpers are owner-owned, `SECURITY DEFINER`, volatile,
   parallel-unsafe, path-pinned and PUBLIC/capability-role revoked. No producer
   or runner receives direct helper execution. The reservation helper locks
   only the authoritative no-FK counter row, validates the workflow policy and
   increments once or raises the registered capacity/policy error; it never
   acquires the workflow control row.

6. **Identity and namespace.** For parent UUID `P` and validated local step `S`,
   SQL derives workflow step `c:<lowercase P>:<S>` and idempotency key
   `chain:<lowercase P>:<S>`. Every producer-supplied idempotency path rejects
   `chain:` as TQ422 reason `reserved_idempotency_namespace`. Migration 0016
   does not activate when a retained `chain:` key lacks matching authoritative
   parent lineage and derivation.

7. **Lifetime reservation and locks.** Every new-contract member insert locks
   and increments the private counter exactly once before insertion. Capacity
   refusal is TQ409 `workflow_member_limit_exceeded` and rolls back the whole
   command. External admit/seal/cancel/finalize use workflow
   `FOR NO KEY UPDATE`; create replay is a plain read; settlement takes no
   workflow control lock. State-count triggers do not alter lifetime admission.
   Seal, cancel and finalization read the exact private state-counter row by
   primary key; they do not rescan a workflow's member table to decide the
   stable terminal predicate. Cancellation enumerates cancelling workflows
   through `workflows_cancel_idx`, then takes at most the remaining command
   budget from each workflow's member frontier.

8. **Claim exclusion and exact plan shape.** Policy-aware claim validates 1–32
   distinct full lowercase hashes and returns null-policy rows plus exact
   matches. Null or empty support returns only null-policy rows. The job policy
   label is engine-stamped from the authoritative workflow at admission.

   WFC-I00 rejects a direct
   `continuation_policy_hash IS NULL OR = ANY(...)` filter: both PostgreSQL
   majors scanned 960,000 unsupported rows in the one-million-row adversarial
   cohort. Normal and affinity claims therefore build one locally ordered,
   batch-bounded frontier for null plus each supported hash, then apply one
   global `priority, scheduled_at, id` top-N. At most
   `(1 + cardinality(supported_hashes)) * batch`, or 1,650 rows, reach that
   final sort. Local branches order by
   `continuation_policy_hash, priority, scheduled_at, id`; equality on the
   leading policy value preserves the existing priority/FIFO semantics.
   Targeted claims continue to use `jobs_pkey`.

9. **Settlement validation order.** After current fence/replay validation and
   before mutation, complete validates the authoritative parent membership and
   policy witness, the entire strict follow-up set, declared queues, two-key
   opt-in, exact derived identities and total remaining capacity. Parent
   success, detached/member child insertions, dependency promotion, state
   counters and lifetime reservation commit together.

10. **Safe reasons.** Direct SQL exposes only the Protocol 1.0.15 closed reason
    set:

    ```text
    TQ422 continuation_parent_not_member
    TQ422 continuation_policy_required
    TQ422 continuation_queue_undeclared
    TQ422 reserved_idempotency_namespace
    TQ409 continuation_policy_mismatch
    TQ409 workflow_member_limit_exceeded
    ```

    Any unregistered detail normalizes to TQ500. Derived-identity inconsistency
    is TQ500, not a caller-selected conflict.

11. **Indexes, constraints and exact names.** WFC-I00 selected the minimum
    sufficient policy-aware family:
    ...
    CREATE INDEX jobs_claim_policy_idx
      ...
      normalized digest 46eb01bf1ce48d2ead1147847a637939
    ...
    CREATE INDEX jobs_affinity_policy_idx
      ...
      normalized digest 71f8d3c1a6b25063b3c1e9bd7094689e
    ...
    CREATE INDEX jobs_workflow_cancel_idx
      ...
      normalized digest 092a4c56bd382e8444bd4b41125eb3df
    ```

    `jobs_claim_idx` remains for the all-policy ready projection.
    `jobs_affinity_policy_idx` replaces, rather than supplements,
    `jobs_affinity_idx`; no other accepted query consumes the old index.
    `jobs_workflow_cancel_idx` is the B9-selected bounded member frontier; it
    prevents a million-member cancellation from scanning and sorting the full
    workflow.

    New constraints are exactly
    ...
    `jobs_continuation_policy_hash_ck`, and
    `jobs_continuation_policy_shape_ck`.

    The exact proposed catalog delta, including dual-major table/constraint
    digests, composite shapes, function identities and metadata, is
    `workflow-continuations/wfc-i00-catalog-delta.json`. Migration 0016 and the
    installed machine manifest now implement this exact 0.2.5 delta while the
    capability remains inactive.

12. **Metadata and compatibility.** ADR-020's supported set becomes exactly
    `{0.1.2,0.1.3,0.1.4,0.1.5,0.2.0,0.2.1,0.2.2,0.2.3,0.2.4,0.2.5}` before
    migration 0016. Migration 0016 retains the exact post-0015 active set:

    ```json
    {"active":["admission_reservations","dependencies_workflows","followups","read_model_list_finished","read_model_list_ready","read_model_list_running","read_model_workflow","schedules","worker_presence"]}
    ```

    Migration 0017 replaces it by exact equality with
    `"workflow_continuations"` appended in lexical order. No other capability
    is implied. Activation/deactivation are immutable metadata migrations,
    never manual DML.

13. **Evidence.** Fresh and full 0001→0017 chains on PostgreSQL 16/18, 0.2.4
    old-client bridge, old/new mixed claims, policy A→A+B→B rollout, every
    WFC-02 barrier race, capacity boundary, million-member B9, fake/HTTP/direct
    parity, auth denial ordering and one usage record, dump/restore, grants,
    exact catalog, installed wheel/sdist extras and the pinned WFC-01C
    cross-repository inventory all gate migration 0017.

## 21. Contract 0.2.6 — trusted host-effect fence (2026-07-30)

ADR-036 adds one co-resident, domain-neutral serialization primitive without
granting any host direct access to TaskQ relations:

```text
taskq.lock_active_effect_attempt(uuid,uuid,text,text,text)
|p_job_id uuid, p_attempt_id uuid, p_worker_id text, p_queue text, p_job_type text
|TABLE(payload jsonb, workflow_id uuid, workflow_counts jsonb)
|plpgsql|v|u|taskq_producer
```

The function is `SECURITY DEFINER`, owned by `taskq_owner`, and uses
`SET search_path = pg_catalog, taskq, pg_temp`. It selects the exact job
`FOR UPDATE` and returns one row only when the job is running, the attempt,
worker, queue and job type all match, the database-clock lease is live, and
cancellation is absent. It returns zero rows for every lost or mismatched
authority. The projection contains only the admitted payload, workflow id, and
the exact six workflow status counts as JSON when the job is a workflow member;
the count object is null for a detached job.

The caller holds the row lock until its surrounding transaction ends. This is
the required atomic boundary for a co-resident host effect: the host validates
the domain subject against the admitted payload and commits the domain mutation
before a concurrent settlement, cancellation or expiry can acquire the same
row. The function grants no claim, heartbeat, release, settlement, observer,
operator, header, progress, result, error, fence or direct-table capability.

Migration `0018_trusted_effect_fence.sql` requires exact SQL contract 0.2.5,
installs the function, advances only `taskq.meta['contract_version']` to
`"0.2.6"`, and retains the exact active capability set. Protocol document
revision stays 1.0.15 and wire major stays 1 because no HTTP identity or wire
shape changes.
