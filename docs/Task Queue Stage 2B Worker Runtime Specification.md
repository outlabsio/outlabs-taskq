...
- `await run_sync(function, *args, **kwargs)` runs a blocking section in the
  supervisor's bounded executor. The supervisor retains the underlying thread
  future if its async caller is cancelled; lease loss, operator-grace expiry,
  or the shutdown deadline then surfaces `requires_process_exit` until that
...
|---|---|---|---|
| heartbeat `ok=false` | cancel immediately; a live managed blocking section becomes `abandoned_sync` | set lease-lost token | suppress forever; live blocking work requires process exit until its thread returns |
...
| soft-stop deadline | hard-cancel; a live managed blocking section requires process exit | signal only; continue heartbeat and wait | async `release(worker_shutdown)` only when no abandoned thread remains; never release live sync work |
...
A synchronous handler, or an async handler's `run_sync` section, may ignore
cancellation and keep the worker from reaching a safe stop. S2-04 never lies
about that. It exposes `requires_process_exit`; S2-05's CLI may exit the
process and let lease expiry reclaim, but it must not release the live attempt
first.
...
6. An exception that violates the typed transport contract is a fail-closed runtime failure: ownership
   becomes unsafe, the handler is cancelled or abandoned under the ordinary async/sync rules, and no
   settlement is attempted. The heartbeat task may never die unobserved while its handler continues.
7. The heartbeat continues through handler normalization and settlement retries. It stops only after a terminal settle outcome or ownership loss.
...
The supervisor owns exactly one `ThreadPoolExecutor(max_workers=sync_workers)`,
created lazily on the first synchronous handler or managed `run_sync` section
and shut down only after no synchronous call remains. The underlying
`concurrent.futures.Future` remains tracked even when an awaiting async task is
cancelled, so a borrowed thread can never become lifecycle-invisible. At most
`concurrency` total handlers and `sync_workers` synchronous calls execute.
Claimed jobs never sit queued inside the executor without an active heartbeat.
...
4. keep live sync handlers heartbeating and wait, or surface
   `requires_process_exit` for a live sync handler or abandoned managed
   blocking section; never release abandoned work;
...
- Direct and supervisor-owned `run_sync` paths check cancellation before and
  after blocking work; supervisor-owned borrowed futures remain visible after
  async cancellation.
...
- `ok=false`, operator cancellation grace, managed blocking abandonment,
  checkpoint retention/flush, missing handler release, and invalid payload
  behavior follow §§4–7.
...
S2-04 is complete only when S2-04A through S2-04D each land as a separate green commit with validation evidence, S2-04-AUDIT makes every acceptance row permanent, the final worktree is clean, and no S2-05 or Stage-3 module exists. The natural round-4 external review boundary is the completed Stage 2B worker kernel plus the contract-0.1.2 upgrade path.
...
The request and response remain separately hard-bounded. Their portable
defaults are 8,192 UTF-8 JSON bytes. A host with a closed, typed batch effect
may raise either `WorkerOptions` ceiling explicitly, up to 8,388,608 bytes;
the selected limits are copied into every `JobContext` by the supervisor.
Direct contexts validate the same finite range. This is a host-runtime bound,
not a change to Protocol payload/result limits: bulky task results still belong
in application tables or artifacts, and a host must size-test its actual effect
models before raising a ceiling.
