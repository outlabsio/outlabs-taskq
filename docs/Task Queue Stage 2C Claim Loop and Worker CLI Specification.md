...
4. Derive once at construction the sorted canonical job-type set whose
   registry entries have callable handlers for this queue, then call
   `transport.claim(queue, worker_id, batch=claim_batch,
   job_types=bound_job_types)` with no targeted job id and no client-side
   timestamp. Handler-free metadata stubs are excluded. A subscribed queue
   with zero bound handlers, or more than Protocol H-09's 20 types, is a
   construction error. The immutable filter is identical over SQL and HTTP;
   `no_handler` release remains defense-in-depth for a stale or misbehaving
   transport, not ordinary fleet routing.
...
8. A retryable typed transport exception means the claim response is unknown, not that the worker
   ...
   whose claim committed before response loss. Non-retryable transport/configuration errors remain
   fatal.
...
S2-05 is complete only when S2-05A through S2-05C land as separate green commits, S2-05-AUDIT makes every acceptance row permanent on both PostgreSQL versions and artifact lanes, each commit records its validation result, the final worktree is clean, and no excluded surface exists.
