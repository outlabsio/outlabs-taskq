...
**Durable admission is the C6 replacement prerequisite:** ADR-023 / Protocol 1.0.8 / SQL contract 0.1.5 generalize the QDarte replay gap into a queue-native reserve/finish/cancel primitive. S5-AR-01 implements the immutable SQL kernel and bridge proof; S5-AR-02 adds generated SQL/HTTP surfaces; S5-AR-AUDIT is the dual-PG acceptance boundary. Accepted immutable release `0.1.0a6` is now exact-hash pinned on the isolated QDarte API/worker branches, and the disposable C6-03B drill proves reserve-before-plan followed by an admitted replay that returns the same job/receipt without replanning or touching the direct/effect ledgers. Targeted acceptance of that evidence gates the already-frozen C6-04 rollback exercises. This sequence adds no permanent host ledger or wrapper, and authorizes no production migration, direct-queue retirement, or Stage-6 work.
