...
**Durable admission is the C6 replacement prerequisite:** ADR-023 / Protocol 1.0.8 / SQL contract 0.1.5 generalize the QDarte replay gap into a queue-native reserve/finish/cancel primitive. S5-AR-01 implements the immutable SQL kernel and bridge proof; S5-AR-02 adds generated SQL/HTTP surfaces; S5-AR-AUDIT is the dual-PG acceptance boundary. Accepted immutable release `0.1.0a6` is exact-hash pinned on the isolated QDarte API/worker branches, and C6-03B proves reserve-before-plan followed by an admitted replay that returns the same job/receipt without replanning or touching the direct/effect ledgers. C6-04 proves all three local zero-row-copy rollback postures: legacy/no-package entry, typed pause/cancel before work, and preserved stable-effect resolution after work. The owner-authorized internal C6-AUDIT re-derived the source paths and exact raw-ledger hashes and accepted the complete local cutover. C7-00 selects Mini87 local production with a separate package database, source-convergence gate, least-privilege identities, capped three-connection increment, two-database restore, a valid one-place allowlist cohort, private Compose facade/worker network, and network-enforced independent egress counter. Round-16's three docs blockers were remediated and internally delta-accepted. C7-01's restricted-runtime preflight, C7-02's exact one-place cohort, and C7-03's two-cycle continuity/zero-insert/backup/restore/rollback evidence are complete and owner-authorized internal Round 19 returned READY. The one package job remains `succeeded / 3 attempts / 2 failures / 0 releases`; direct and domain ledgers stayed byte-identical, and production is paused/draining. The C8 retirement proposal is frozen around a caller floor, producer-first and consumer-second windows, a stale-worker no-claim guard, shared-ledger preservation, and paired zero-DML rollback. Internal Round 20 accepted that architecture and the docs remediation now freezes the actual safe service transition, measures historical direct workloads at 1–293 entities, imposes one-deep/one-running server controls, stages 25/100/300 package cohorts before the corresponding caller cap, and adopts exact-ID status with operator-only cancellation. Targeted delta review must return READY before C8-R1; another lane and Stage 6 remain closed.

**C8 retirement design accepted:** the owner-authorized internal Round-20 delta closes the actual-safe-posture, workload-envelope, and UI-authority findings with zero Contract questions. C8-R1 still cannot change a caller, IAM, service, queue, or production setting until the next naturally scheduled 03:15 backup and the remaining eligibility evidence pass. R1 then implements only the disabled-first caller/exact-ID status floor; producer and consumer removal retain their own later windows and acceptance gates.
