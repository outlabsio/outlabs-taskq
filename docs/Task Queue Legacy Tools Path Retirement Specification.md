...
7. zero active legacy `tool_run` rows (`pending` or `running`) at the removal boundary.
...
- reject or remove `TASKQ_TOOLS_MODE`; `TASKQ_ENABLED` remains the runtime mount/readiness control,
  not permission to publish through the retired path;
...
- retain explicit handling that quarantines or fails loudly on an unexpected historical `tool_run`
  row rather than executing it silently;
...
|---|---|
| LR-01 | Seven-day/two-deploy eligibility ledger and independent taskq/external invocation reconciliation |
| LR-02 | Frozen high-water oracle proves zero new legacy `tool_run` rows and zero active rows at cutover |
| LR-03 | ASGI/source tests make the tools legacy publisher unreachable in disabled, error, and ambiguous cases |
...
| LR-07 | L3 removes only `tool_run` consumption and fails loudly on an unexpected historical row |
...
cannot fail closed without changing Protocol v1, if non-tools work shares an inseparable dispatch
surface, if rollback needs row mutation, or if an acceptance oracle requires payload/fence/credential
exposure. Record taskq contract conflicts in `TASKS.md`; record host topology questions in the host
completion packet. Do not expand this slice to side-effecting lanes.
