...
def _runtime(
    ...
    *,
    ...
    resources = TaskqFacadeTransports(
        ...
        worker_presence_enabled=bool(options and options.worker_presence_enabled),
    )
    kwargs: dict[str, Any] = {}
...
async def test_runtime_bridge_accepts_closed_contract_set_and_keeps_prebridge_rejection(
    ...
    assert exc_info.value.details == {"contract_version": "0.2.4"}
...
def test_schedule_surface_is_explicitly_disabled_by_default() -> None:
    ...
    assert TaskqRuntimeOptions().worker_presence_enabled is False
    assert "schedule_operator" not in TaskqFacadeTransports.__dataclass_fields__


async def test_worker_presence_runtime_requires_0_2_4_and_exact_capability() -> None:
    options = TaskqRuntimeOptions(
        housekeeper_enabled=False,
        long_poll_listener_enabled=False,
    ...
    wrong = _runtime(
        _Transport(version="0.2.3", capabilities={"active": ["worker_presence"]}),
        options=options,
    )
    with pytest.raises(TaskqVersionError):
        await wrong.start()

    missing = _runtime(_Transport(version="0.2.4"), options=options)
    with pytest.raises(TaskqCapabilityError):
        await missing.start()

    active = _runtime(
        _Transport(version="0.2.4", capabilities={"active": ["worker_presence"]}),
        options=options,
    )
    await active.start()
    assert active.state is TaskqRuntimeState.RUNNING
