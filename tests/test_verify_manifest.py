...
CASES = (
    ...
    Corruption(
        "schedule_name_bound",
        (
            "ALTER TABLE taskq.schedules DROP CONSTRAINT schedules_name_ck",
            """
            ALTER TABLE taskq.schedules ADD CONSTRAINT schedules_name_ck CHECK (
                octet_length(name) >= 1
                AND octet_length(name) <= 119
...
def test_machine_manifest_has_closed_0_5_0_function_surface() -> None:
    ...
    assert "taskq.list_workflows(text,integer,jsonb)" in FUNCTIONS
    assert "taskq.list_schedules(text,integer,text)" in FUNCTIONS
