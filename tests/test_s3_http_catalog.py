...
def test_h09_json_and_collection_bounds_are_enforced_by_wire_models() -> None:
    ...
    with pytest.raises(ValidationError):
        WorkerPresenceWireRequest(
            worker_id="worker-1", queues=tuple(f"queue_{index}" for index in range(33))
        )
    with pytest.raises(ValidationError, match="invalid queue"):
        WorkerPresenceWireRequest(worker_id="worker-1", queues=("Not_Canonical",))
    with pytest.raises(ValidationError):
        WorkerPresenceWireRequest(worker_id="worker-1", queues=("one",), version="v" * 201)
