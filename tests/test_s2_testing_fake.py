...
from taskq.errors import TaskqConfigError, TaskqConflictError, TaskqValidationError
...
@pytest.mark.asyncio
async def test_fake_worker_presence_matches_bounded_live_page_contract() -> None:
    instant = [datetime(2026, 7, 25, 12, 0, tzinfo=UTC)]
    fake = FakeTaskQClient(queues=("presence",), clock=lambda: instant[0])

    await fake.worker_heartbeat("worker-a", ("presence",), version="v1")
    instant[0] += timedelta(seconds=1)
    await fake.worker_heartbeat("worker-b", ("presence",), version=None)
    job = await fake.enqueue(
        EnqueueCommand(queue="presence", job_type="tests.presence", payload={})
    )
    claimed = await fake.claim("presence", "worker-b", job_id=job.job_id)
    assert claimed.state is ClaimState.CLAIMED

    first = await fake.list_worker_presence(limit=1)
    assert first.as_of == instant[0]
    assert len(first.items) == 1
    assert first.items[0].worker_id == "worker-b"
    assert first.items[0].declared_queues == ("presence",)
    assert first.items[0].version is None
    assert first.items[0].online is True
    assert first.items[0].running_jobs == 1
    assert first.next_last_seen_at == first.items[0].last_seen_at
    assert first.next_worker_id == "worker-b"

    second = await fake.list_worker_presence(
        limit=1,
        after_last_seen_at=first.next_last_seen_at,
        after_worker_id=first.next_worker_id,
    )
    ...
    instant[0] += timedelta(seconds=1)
    await fake.worker_heartbeat("worker-a", ("presence",), version="v2")
    omitted = await fake.list_worker_presence(
        limit=100,
        after_last_seen_at=first.next_last_seen_at,
        after_worker_id=first.next_worker_id,
    )
    ...
    with pytest.raises(TaskqValidationError) as invalid:
        await fake.worker_heartbeat("worker-c", ("presence", "presence"))
    assert invalid.value.details == {"field": "queues"}
