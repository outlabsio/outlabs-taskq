...
async def test_producer_cannot_preoccupy_derived_followup_namespace(
    operator: asyncpg.Connection,
    producer: asyncpg.Connection,
) -> None:
    ...
    with pytest.raises(asyncpg.PostgresError) as excinfo:
        await producer.fetchrow(
            ...
            "p_idempotency_key => 'chain:reserved:child')",
            queue,
        )
    assert excinfo.value.sqlstate == "TQ422"
    assert excinfo.value.detail == '{"reason":"reserved_idempotency_namespace"}'
