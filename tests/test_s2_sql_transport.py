...
def test_transport_method_ledger_is_exactly_the_public_manifest() -> None:
    ...
    assert len(METHOD_FUNCTIONS) == 49
    assert len(PUBLIC_FUNCTIONS) == 60
...
async def test_wfc_i03_new_overloads_refuse_while_capability_is_inactive(
    ...
    try:
        ...
        with pytest.raises(TaskqCapabilityError):
            await transports["runner"].claim(
                queue,
                "policy-worker",
                supported_policy_hashes=("a" * 64,),
            )

        await _enqueue(transports, queue)
        job_id, attempt_id = await _claim(transports, queue, "policy-worker")
        with pytest.raises(TaskqCapabilityError):
            await transports["runner"].complete(
                job_id,
                attempt_id,
                "policy-worker",
                continuation_policy_hash="a" * 64,
...
async def test_sql_transport_has_no_background_tasks_or_checked_out_resources(
    ...
    assert (await transport.get_contract_meta()).contract_version == "0.3.0"
