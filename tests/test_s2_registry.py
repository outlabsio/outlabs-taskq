...
def test_workflow_enqueue_shape_is_closed_before_transport() -> None:
    workflow_id = uuid4()
    dependency = uuid4()
    valid = EnqueueCommand(
        queue="default",
        job_type="math.double",
        payload={"value": 3},
    ...
    assert valid.workflow_id == workflow_id
    assert valid.depends_on == (dependency,)
