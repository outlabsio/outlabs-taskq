...
class FacadeTransport:
    ...
    async def list_worker_presence(self, **kwargs: Any) -> WorkerPresencePage:
        self.calls.append(("list_worker_presence", kwargs))
        instant = datetime(2026, 7, 25, 12, 0, tzinfo=UTC)
        after = kwargs["after_last_seen_at"]
        return WorkerPresencePage(
            ...
            next_last_seen_at=instant if after is None else None,
            next_worker_id="worker-1" if after is None else None,
        )
...
async def test_complete_authenticates_once_and_authorizes_sorted_distinct_actual_queues() -> None:
    job_id = uuid4()
    transport = FacadeTransport()
    transport.projections[job_id] = AuthorizationProjection(
        job_id=job_id, queue="parents", job_type="tests.parent", status=JobStatus.RUNNING
    ...
    async with httpx.AsyncClient(
        ...
        response = await client.post(
            ...
            json={
                "attempt_id": str(uuid4()),
                "worker_id": "worker",
                "continuation_policy_hash": "a" * 64,
                "followups": [
                    {"step": "z", "job_type": "tests.z", "queue": "zeta"},
                    {"step": "same", "job_type": "tests.same"},
                    {"step": "a", "job_type": "tests.a", "queue": "alpha"},
                    {"step": "z2", "job_type": "tests.z2", "queue": "zeta"},
                ],
            },
        )
    ...
    assert checked == ["parents", "alpha", "zeta"]
    complete = [payload for name, payload in transport.calls if name == "complete"]
    assert len(complete) == 1
    assert complete[0][1]["continuation_policy_hash"] == "a" * 64


async def test_continuation_wire_bounds_reject_before_any_taskq_call() -> None:
    transport = FacadeTransport()
    job_id = uuid4()
    transport.projections[job_id] = AuthorizationProjection(
        job_id=job_id,
    ...
    async with httpx.AsyncClient(
        ...
        claim = await client.post(
            "/taskq/v1/queues/parents/claims",
            headers=_headers(),
            json={
                "worker_id": "worker",
                "supported_policy_hashes": [f"{index:064x}" for index in range(33)],
            },
        )
        complete = await client.post(
            ...
            json={
                "attempt_id": str(uuid4()),
                "worker_id": "worker",
                "continuation_policy_hash": "NOT-A-HASH",
            },
        )

    assert claim.status_code == complete.status_code == 422
    assert claim.json()["error"]["code"] == "TQ422"
    assert complete.json()["error"]["code"] == "TQ422"
    assert transport.calls == [("projection", job_id)]


async def test_omitted_workflow_fields_preserve_legacy_custom_transport() -> None:
    class LegacyWorkflowProducer:
        def __init__(self) -> None:
    ...
    transport = FacadeTransport()
    workflow = LegacyWorkflowProducer()
    resources = replace(
        _resources(transport),
        workflow_producer=workflow,  # type: ignore[arg-type]
        workflow_authorization=transport,  # type: ignore[arg-type]
...
async def test_worker_presence_authorizes_before_cursor_and_round_trips_page() -> None:
    transport = FacadeTransport()

    async def authenticate(request: Any) -> AuthContext:
        return AuthContext(actor="observer", principal="observer")
    ...
    denied_resources = replace(_resources(transport), worker_presence_enabled=True)
    denied_app = _mounted(
        create_taskq_app(
            denied_resources,
            authorizer=callable_auth(authenticate, deny),
    ...
    async with httpx.AsyncClient(
        ...
        denied = await client.get(
            "/taskq/v1/workers?cursor=not-base64!",
            headers=_headers(),
        )
    assert denied.status_code == 403
    assert all(name != "list_worker_presence" for name, _ in transport.calls)

    enabled_resources = replace(_resources(transport), worker_presence_enabled=True)
    enabled_app = _mounted(create_taskq_app(enabled_resources, authorizer=no_auth_for_tests()))
    async with httpx.AsyncClient(
        transport=httpx.ASGITransport(app=enabled_app), base_url="http://test"
    ) as client:
        first = await client.get("/taskq/v1/workers?limit=1", headers=_headers())
        cursor = first.json()["data"]["next_cursor"]
        second = await client.get(
            f"/taskq/v1/workers?limit=1&cursor={cursor}",
            headers=_headers(),
        )
    ...
    assert malformed.status_code == 422
    calls = [payload for name, payload in transport.calls if name == "list_worker_presence"]
    assert calls[0]["after_last_seen_at"] is None
    assert calls[0]["after_worker_id"] is None
    assert calls[1]["after_last_seen_at"] == datetime(2026, 7, 25, 12, 0, tzinfo=UTC)
    assert calls[1]["after_worker_id"] == "worker-1"
