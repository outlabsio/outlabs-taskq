...
async def test_workflow_count_transitions_and_active_page(
    ...
    assert tuple(counts) == (workflow_id, 0, 1, 0, 0, 0, 0, 1)
    ...
    assert counts is not None
    assert tuple(counts)[1:] == (0, 0, 1, 0, 0, 0, 1)
    ...
    assert counts is not None
    assert tuple(counts)[1:] == (0, 0, 0, 1, 0, 0, 1)
...
async def test_0011_backfills_counts_and_full_verify(taskq_dsn: str) -> None:
    ...
    try:
        ...
        async with engine.connect() as conn:
            ...
            applied = await conn.run_sync(
                lambda sync_conn: _migrate_impl(sync_conn, migrations[13:14])
            )
            assert applied == ["0014_worker_presence_projection"]
            applied = await conn.run_sync(
                lambda sync_conn: _migrate_impl(sync_conn, migrations[14:15])
            )
            assert applied == ["0015_activate_worker_presence"]
            applied = await conn.run_sync(
                lambda sync_conn: _migrate_impl(sync_conn, migrations[15:16])
            )
            assert applied == ["0016_workflow_continuations"]
            applied = await conn.run_sync(
