...
async def test_borrowed_async_client_uses_taskq_timeout_per_request() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return _response(
            request,
            data={"contract_version": "0.2.5", "capabilities": {}},
        )
    ...
    assert seen_timeout == {"connect": 30, "read": 30, "write": 30, "pool": 30}


def test_borrowed_sync_client_uses_taskq_timeout_per_request() -> None:
    seen_timeout: dict[str, float] = {}
...
async def test_claim_transport_failure_is_never_replayed_after_negotiation() -> None:
    ...
    async with httpx.AsyncClient(
        ...
        with pytest.raises(TaskqUnavailableError) as unavailable:
            await client.claim("emails", "worker-1")
    assert claim_calls == 1
    assert isinstance(unavailable.value.cause, httpx.ReadError)


@pytest.mark.parametrize("client_type", [AsyncTaskqHttpClient, TaskqHttpClient])
def test_http_transport_failure_is_normalized_for_both_clients(
    client_type: type[AsyncTaskqHttpClient] | type[TaskqHttpClient],
) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        raise httpx.ConnectError("offline", request=request)

    if client_type is AsyncTaskqHttpClient:
        ...
        async def run() -> None:
            async with httpx.AsyncClient(
                ...
                with pytest.raises(TaskqUnavailableError) as unavailable:
                    await client.get_contract_meta()
                assert isinstance(unavailable.value.cause, httpx.ConnectError)
