...
@pytest.mark.parametrize(
    ("reason", "fatal"),
    [
        ("workflow_member_limit_exceeded", False),
        ("continuation_policy_mismatch", True),
    ],
)
async def test_permanent_continuation_conflict_terminal_fails_parent(
    reason: str, fatal: bool
) -> None:
    clock = ManualClock()
    transport = ScriptedTransport()
    transport.script("complete", TaskqConflictError(details={"reason": reason}))
...
async def test_unclassified_continuation_conflict_remains_fatal() -> None:
    clock = ManualClock()
    transport = ScriptedTransport()
    transport.script("complete", TaskqConflictError(details={"reason": "unknown"}))
    supervisor = _supervisor(transport, clock, followup_handler)
    report = await supervisor.run_job(_claim())
    assert report.outcome is JobRunOutcome.RUNTIME_ERROR
    assert report.fatal
