...
class BlockingCommittedReporter(RecordingReporter):
...
class UnavailableReporter(RecordingReporter):
    async def report_effect(
        self, attempt: WorkerEffectAttempt, request: dict[str, Any]
    ) -> dict[str, Any]:
        self.calls.append((attempt, request))
...
def _supervisor(
    ...
    return WorkerSupervisor(
        ...
        options=WorkerOptions(
            ...
            settle_backoff_base=0.25,
            settle_backoff_cap=1,
        ),
        clock=clock,
...
async def test_reporter_replays_the_same_request_after_response_loss() -> None:
    ...
    assert supervisor.active_slots == 0
    await supervisor.aclose()


async def test_reporter_retry_exhaustion_is_bounded_and_releases_resources() -> None:
    async def handler(ctx: JobContext, payload: Input) -> Output:
        await ctx.report_effect({"operation": "contact_apply", "value": payload.value})
        raise AssertionError("unavailable reporter must not return")
    ...
    clock.advance(0.25)
    await _spin_until(lambda: len(reporter.calls) == 2)
    await _spin_until(lambda: clock.sleeping == 2)
    clock.advance(0.5)
    await _spin_until(lambda: len(reporter.calls) == 3)
    report = await running

    assert report.state is JobRunState.SETTLED
    assert len(reporter.calls) == 3
    assert reporter.calls[0] == reporter.calls[1] == reporter.calls[2]
...
async def test_operator_cancellation_before_report_prevents_the_effect() -> None:
    entered = asyncio.Event()
    allow_report = asyncio.Event()
    ...
    assert report.state is JobRunState.SETTLED
    assert report.cancellation_reason is not None
...
async def test_operator_cancellation_after_committed_report_never_accepts_a_child() -> None:
    async def handler(ctx: JobContext, payload: Input) -> Output:
        await ctx.report_effect({"operation": "contact_apply", "value": payload.value})
        raise AssertionError("cancelled report must not return effect truth to the handler")
    ...
    reporter = BlockingCommittedReporter()
    supervisor = _supervisor(handler, transport, clock, reporter)
    running = asyncio.create_task(supervisor.run_job(_claim()))

    await reporter.committed.wait()
    await _spin_until(lambda: clock.sleeping == 1)
    clock.advance(5)
    await _spin_until(lambda: transport.calls and transport.calls[0].command == "heartbeat")
    reporter.allow_return.set()
    report = await running
