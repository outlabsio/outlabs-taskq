...
async def test_direct_context_run_sync_checks_cancellation_around_blocking_work() -> None:
    context = _context()
    assert await context.run_sync(lambda left, right: left + right, 2, 3) == 5
    context.cancellation.cancel(CancellationReason.OPERATOR)
    with pytest.raises(TaskCancelled) as exc_info:
        await context.run_sync(lambda: None)
...
async def test_context_effect_reporter_supports_explicit_larger_hard_bounds() -> None:
    async def reporter(request: dict[str, Any]) -> dict[str, Any]:
        assert len(str(request["payload"])) == 16_000
        return {"payload": "y" * 24_000}

    context = JobContext(
        job_id=uuid4(),
        queue="worker",
        job_type="batch.apply",
        payload=Input(value=2),
        headers=None,
        progress=None,
        attempt_number=1,
    ...
    result = await context.report_effect({"payload": "x" * 16_000})
    assert len(str(result["payload"])) == 24_000

    with pytest.raises(TaskqConfigError, match="configured 32KB"):
        await context.report_effect({"payload": "x" * (32 * 1024)})


@pytest.mark.parametrize(
    ("field", "value"),
    (
        ("effect_request_max_bytes", 1023),
        ("effect_request_max_bytes", 8_388_609),
        ("effect_response_max_bytes", 1023),
        ("effect_response_max_bytes", 8_388_609),
    ),
)
def test_context_effect_bounds_are_finite(field: str, value: int) -> None:
    with pytest.raises(TaskqConfigError, match=field):
        JobContext(
            job_id=uuid4(),
            queue="worker",
            job_type="batch.apply",
