...
class _DepsProxy:
    def __init__(self, deps: AuthDeps) -> None:
        self._deps = deps
        self.authorization_names: list[tuple[str, ...]] = []
    ...
    def authenticated_authorization_requires_session(
        ...
        return self._deps.authenticated_authorization_requires_session(auth_result)
    ...
    async def authorize_authenticated(
        ...
        **kwargs: Any,
        ...
        self.authorization_names.append(names)
        return await self._deps.authorize_authenticated(
            request,
            auth_result,
            *names,
            **kwargs,
...
async def test_real_permission_dependency_queue_global_denial_actor_and_cache() -> None:
    ...
    assert len(set(deps.authorization_names)) == 4
    assert len(finalized) == 12  # authentication only; service-token authz is session-free


async def test_authenticated_context_reuses_one_authentication_for_five_authorizations() -> None:
    finalized: list[object] = []
    ...
    permissions = [
        "taskq_emails:enqueue",
        "taskq_emails:run",
        "taskq_emails:read",
        "taskq_emails:control",
        "taskq_emails:admin",
    ]
    authorizer, deps = _real_auth(permissions, sessions)
    request = _request()
    context = await authorizer.authenticate(request)

    for action in TaskqAction:
        await authorizer.authorize_context(request, context, action, "emails")

    assert len(finalized) == 1
    assert deps.authorization_names == [
        (f"taskq_emails:{action.value}", f"taskq:{action.value}") for action in TaskqAction
    ]


async def test_authenticated_context_is_request_bound() -> None:
    finalized: list[object] = []
    ...
    authorizer, deps = _real_auth(["taskq_emails:run"], sessions)
    context = await authorizer.authenticate(_request())

    with pytest.raises(HTTPException) as denied:
        await authorizer.authorize_context(
            _request(),
            context,
            TaskqAction.RUN,
            "emails",
...
async def test_user_owned_context_opens_policy_session_for_authorization() -> None:
    session_scopes = 0
    auth_result = {
        "source": "api_key",
        "user_id": str(uuid4()),
        "metadata": {"scopes": ["taskq_emails:run"]},
    }

    async def checker(request: Request, session: Any) -> dict[str, Any]:
        return auth_result

    async def sessions() -> Any:
        nonlocal session_scopes
        session_scopes += 1
        yield _OwnedSession([])
    ...
    async def authorize_authenticated(
        ...
        **kwargs: Any,
        ...
        assert names == ("taskq_emails:run", "taskq:run")
        assert session is not None
        return result

    deps = SimpleNamespace(
        require_auth=lambda: checker,
        authenticated_authorization_requires_session=lambda result: True,
        authorize_authenticated=authorize_authenticated,
    )
    ...
    request = _request()
    context = await authorizer.authenticate(request)
    await authorizer.authorize_context(request, context, TaskqAction.RUN, "emails")

    assert session_scopes == 2
...
async def test_adapter_sanitizes_real_dependency_http_failures(status: int, reason: str) -> None:
    ...
    async def authorize_authenticated(*args: Any, **kwargs: Any) -> Any:
        raise AssertionError("authentication failure must stop before authorization")

    deps = SimpleNamespace(
        require_auth=lambda: checker,
        authenticated_authorization_requires_session=lambda result: False,
        authorize_authenticated=authorize_authenticated,
    )
    auth = SimpleNamespace(deps=deps)
...
def test_catalog_is_sorted_exact_validated_and_rejects_noncanonical_queue() -> None:
    # The release lock verifies one exact compatible artifact even though the
    # public optional dependency permits independently released 0.1 patches.
    assert importlib.metadata.version("outlabs-auth") == "0.1.0a28"
