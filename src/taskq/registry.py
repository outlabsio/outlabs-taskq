...
from typing import TYPE_CHECKING, Any, Generic, TypeAlias, TypeVar, Union, get_args, get_origin
...
if TYPE_CHECKING:
    from taskq.continuations import CompiledContinuationPolicy
...
class FollowupTarget(BaseModel):
    ...
    workflow_member: bool = Field(default=False, strict=True)
    continuation_revision: str | None = Field(
        default=None,
        min_length=1,
        max_length=64,
    ...
    @model_validator(mode="after")
    def _member_revision_pair(self) -> FollowupTarget:
        if self.workflow_member != (self.continuation_revision is not None):
            raise ValueError(
                "continuation_revision is required exactly when workflow_member is true"
            )
        return self
...
class Task(Generic[InT, OutT]):
    ...
    def __post_init__(self) -> None:
        ...
        revisions = {target.continuation_revision for target in targets if target.workflow_member}
        if len(revisions) > 1:
            raise TaskqConfigError(
                "member-enabled followup_targets must share one continuation revision"
            )
...
class TaskRegistry:
    ...
    def validate_followup_graph(self) -> None:
        ...
        for parent in self._canonical.values():
            ...
            for declared in parent.followup_targets:
                child = self.resolve(declared.job_type)
                if child is None:
                    raise TaskqConfigError(
                ...
                identity = (declared.queue, child.name)
                if identity in identities:
                    raise TaskqConfigError(
                        f"followup targets are not canonically distinct for parent: {parent.name!r}"
                    )
    ...
    def compile_continuation_policy(
        ...
        """Compile this registry's member-enabled graph into an immutable policy."""

        from taskq.continuations import compile_continuation_policy

        return compile_continuation_policy(self, roots)
    ...
    def normalize_followups(
        ...
        declared: dict[tuple[str, str], FollowupTarget] = {}
        for item in parent.followup_targets:
            target = self.resolve(item.job_type)
            if target is None:
                raise TaskqConfigError(f"followup target is not registered: {item.job_type!r}")
            identity = (item.queue, target.name)
        ...
        for followup in followups:
            ...
            declaration = declared.get(identity)
            if target.queue != queue or declaration is None:
                raise InvalidFollowupError(
                    f"followup target is not declared for parent: {identity!r}"
                )
            if followup.workflow_member is True and not declaration.workflow_member:
                raise InvalidFollowupError(
                    f"followup target is not member-enabled for parent: {identity!r}"
                )
            try:
