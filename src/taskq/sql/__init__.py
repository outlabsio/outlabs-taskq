...
def _execute_migration_statement(conn: Connection, statement: str) -> None:
    """Preserve literal percent characters across DBAPI parameter styles."""

    if conn.dialect.paramstyle in {"format", "pyformat"}:
...
def _check_constraints(conn: Connection) -> VerifyCheck:
    ...
    actual = {row["relname"]: row for row in rows}
    details = _set_details(
        "constraints",
        set(_manifest.CONSTRAINTS),
        set(actual),
    )
    for relname in sorted(set(_manifest.CONSTRAINTS) & actual.keys()):
        expected_count, canonical_digest = _manifest.CONSTRAINTS[relname]
        accepted_digests = {
            canonical_digest,
            *_manifest.CONSTRAINT_EQUIVALENT_DIGESTS.get(relname, ()),
        }
        got_count = actual[relname]["item_count"]
        got_digest = actual[relname]["digest"]
        if got_count != expected_count or got_digest not in accepted_digests:
            details.append(f"constraints '{relname}' definition differs from manifest")
    return VerifyCheck(
        name="constraints",
        ok=not details,
