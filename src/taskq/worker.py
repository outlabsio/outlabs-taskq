...
_PERMANENT_CONTINUATION_CONFLICTS = frozenset(
    {
...
class JobRunOutcome(StrEnum):
    ...
    UNSUPPORTED_CONTINUATION_POLICY = "unsupported_continuation_policy"
...
class WorkerService:
    ...
    def __init__(
        ...
        *,
        ...
        policies = tuple(continuation_policies)
        policy_hashes = tuple(sorted(policy.continuation_policy_hash for policy in policies))
        if len(set(policy_hashes)) != len(policy_hashes):
            raise TaskqConfigError("continuation policies must have distinct hashes")
        if len(policy_hashes) > MAX_CONTINUATION_POLICY_HASHES:
            raise TaskqConfigError(
                f"workers support at most {MAX_CONTINUATION_POLICY_HASHES} continuation policies"
            )
        bound_by_queue: dict[str, tuple[str, ...]] = {}
        for queue in options.queues:
            names = tuple(
                sorted(
                    task.name
                    for task in registry
                    if task.queue == queue and task.handler is not None
                )
            )
            if not names:
                raise TaskqConfigError(f"subscribed queue has no bound handlers: {queue!r}")
            if len(names) > 20:
                raise TaskqConfigError(
                    f"subscribed queue exceeds the 20 job-type claim limit: {queue!r}"
                )
            bound_by_queue[queue] = names
    ...
    def claim_job_types(self, queue: str) -> tuple[str, ...]:
        """Return the exact immutable claim filter for one subscribed queue."""

        try:
            return self._job_types_by_queue[queue]
        except KeyError as exc:
            raise TaskqConfigError(f"worker is not subscribed to queue: {queue!r}") from exc
    ...
    def _observe_job(self, completed: asyncio.Task[JobRunReport]) -> None:
        ...
        if report.fatal:
            ...
            logger.error(
                ...
                extra={"worker_id": self.worker_id, "job_id": str(report.job_id)},
            )
            self._stop_requested.set()
            self._nudge.nudge()
            self._ensure_stop_task()
    ...
    async def _write_presence(self) -> bool:
        ...
        if self._supported_policy_hashes:
            meta["continuation_policy_hashes"] = list(self._supported_policy_hashes)
        try:
...
class _RunControl:
    def __init__(self, *, is_sync: bool) -> None:
        ...
        self.blocking_futures: set[ConcurrentFuture[Any]] = set()
...
class WorkerSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        policies = tuple(continuation_policies)
        policy_hashes = tuple(policy.continuation_policy_hash for policy in policies)
        if len(set(policy_hashes)) != len(policy_hashes):
            raise TaskqConfigError("continuation policies must have distinct hashes")
        if len(policy_hashes) > MAX_CONTINUATION_POLICY_HASHES:
            raise TaskqConfigError(
        ...
        for policy in policies:
            validate_compiled_continuation_policy(registry, policy)
        self.transport = transport
        self.registry = registry
        self.worker_id = worker_id
        ...
        self._supported_policy_hashes = frozenset(self._continuation_policies)
        ...
        self._unsafe_blocking: set[ConcurrentFuture[Any]] = set()
        self._stop_task: asyncio.Task[None] | None = None
    ...
    async def _run_reserved(self, claim: ClaimedJob, key: tuple[UUID, UUID]) -> JobRunReport:
        ...
        try:
            ...
            if fatal and self._stop_task is None:
                self._stop_task = asyncio.create_task(self._stop(), name="taskq-worker-stop")
            if soft_stop and self._stop_task is None:
                self._stop_task = asyncio.create_task(self._stop(), name="taskq-worker-stop")
    ...
    async def _execute_job(self, claim: ClaimedJob, key: tuple[UUID, UUID]) -> JobRunReport:
        continuation_policy = (
            None
            if claim.continuation_policy_hash is None
            else self._continuation_policies.get(claim.continuation_policy_hash)
        )
        if claim.continuation_policy_hash is not None and continuation_policy is None:
            return await self._release_unsupported_policy(claim)
        ...
        async def run_sync(
            ...
            return await self._run_sync(control, function, args, kwargs)
        ...
        context = JobContext(
            ...
            workflow_step=claim.step_key,
            effect_reporter=report_effect if self.effect_reporter is not None else None,
            effect_request_max_bytes=self.options.effect_request_max_bytes,
            effect_response_max_bytes=self.options.effect_response_max_bytes,
            blocking_runner=run_sync,
        )
        ...
        try:
            ...
            if (
                ...
                if not control.is_sync and not control.handler.done():
                    self._mark_blocking_unsafe(control)
        ...
        try:
            intent = self._normalize_handler_result(
                task,
                handler_value,
                handler_error,
    ...
    async def _run_sync(
        ...
        loop = asyncio.get_running_loop()
        future = self._executor_for_sync().submit(function, *args, **kwargs)
        control.blocking_futures.add(future)

        def finished(completed: ConcurrentFuture[Any]) -> None:
            loop.call_soon_threadsafe(self._finish_blocking, control, completed)
    ...
    def _finish_blocking(
        ...
        control.blocking_futures.discard(future)
        self._unsafe_blocking.discard(future)

    def _mark_blocking_unsafe(self, control: _RunControl) -> None:
        pending = {future for future in control.blocking_futures if not future.done()}
        if pending:
            control.abandoned_sync = True
            self._unsafe_blocking.update(pending)
    ...
    def _normalize_handler_result(
        ...
        *,
        continuation_policy: CompiledContinuationPolicy | None,
    ) -> HandlerResult:
        ...
        if isinstance(value, HANDLER_RESULT_TYPES):
            if isinstance(value, Complete):
                ...
                followups = enforce_continuation_policy(
                    task,
                    followups,
                    continuation_policy,
                )
    ...
    async def _release_unsupported_policy(self, claim: ClaimedJob) -> JobRunReport:
        async def operation() -> SettleResult:
            return await self.transport.release(
                claim.job_id,
                claim.attempt_id,
                self.worker_id,
                "released",
                delay_seconds=self.options.unsupported_policy_delay_seconds,
                progress=claim.progress,
            )
        ...
        if report.state is JobRunState.SETTLED:
            return report.model_copy(
                update={"outcome": JobRunOutcome.UNSUPPORTED_CONTINUATION_POLICY}
            )
        return report
    ...
    async def _settle_intent(
        ...
        *,
        ...
        if intent is None:
            ...
            async def operation() -> SettleResult:
                complete_options: dict[str, Any] = {
                    "result": intent.result,
                    "followups": intent.followups,
                }
                if claim.continuation_policy_hash is not None:
                    complete_options["continuation_policy_hash"] = claim.continuation_policy_hash
        ...
        try:
        ...
        except (TaskqValidationError, TaskqCapabilityError, TaskqConflictError) as exc:
            if not isinstance(intent, Complete) or not intent.followups:
                return self._runtime_failure(claim.job_id, command)
            if isinstance(exc, TaskqConflictError) and (
                exc.details.get("reason") not in _PERMANENT_CONTINUATION_CONFLICTS
            ):
                return self._runtime_failure(claim.job_id, command)
            report = await self._settle_invalid_followup(
                ...
                progress,
                capability_skew=(
                    isinstance(exc, TaskqCapabilityError)
                    or (
                        isinstance(exc, TaskqConflictError)
                        and exc.details.get("reason") == "continuation_policy_mismatch"
                    )
                ),
                control=control,
            )
        ...
        *,
        ...
        for attempt in range(1, self.options.settle_max_attempts + 1):
            ...
            try:
                ...
                raise
            except TaskqConflictError as exc:
                if (
                    command is CommandName.COMPLETE
                    and exc.details.get("reason") in _PERMANENT_CONTINUATION_CONFLICTS
                ):
                    raise
