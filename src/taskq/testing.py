...
class FakeTaskQClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._workers: dict[str, _FakeWorkerPresence] = {}
    ...
    async def _enqueue_workflow_member(self, command: EnqueueCommand) -> EnqueueResult:
        ...
        if workflow.member_limit is not None and workflow.admitted_total >= workflow.member_limit:
            raise TaskqConflictError(details={"reason": "workflow_member_limit_exceeded"})
        ...
        job = _FakeJob(
            ...
            command=command,
            status=status,
            continuation_policy_hash=workflow.continuation_policy_hash,
        )
        self._jobs[job.job_id] = job
        self._order.append(job.job_id)
        workflow.steps[command.step_key] = job.job_id
        workflow.admitted_total += 1
        self._known_queues.add(command.queue)
        if command.idempotency_key is not None:
        ...
        *,
        ...
        if existing_id is not None:
            ...
            if (
                ...
                or workflow.member_limit
                != (continuation.member_limit if continuation is not None else None)
                or workflow.continuation_policy_hash
                != (continuation.continuation_policy_hash if continuation is not None else None)
            ):
        ...
        workflow = _FakeWorkflow(
            ...
            member_limit=(continuation.member_limit if continuation is not None else None),
        ...
        *,
        ...
        for candidate_id in self._order:
            ...
            if affinity_key is not None and command.affinity_key != affinity_key:
                continue
            if candidate.continuation_policy_hash is not None and (
                continuation is None
                or candidate.continuation_policy_hash not in continuation.supported_policy_hashes
            ):
                continue
            ...
            selected.append(
                ClaimedJob(
                    ...
                    continuation_policy_hash=candidate.continuation_policy_hash,
                )
            )
    ...
    async def complete(
        ...
        *,
        ...
        if job.continuation_policy_hash is not None:
            if continuation_policy_hash is None:
                raise TaskqValidationError(details={"reason": "continuation_policy_required"})
            if continuation_policy_hash != job.continuation_policy_hash:
                raise TaskqConflictError(details={"reason": "continuation_policy_mismatch"})
        elif continuation_policy_hash is not None:
            raise TaskqConflictError(details={"reason": "continuation_policy_mismatch"})
        children: list[tuple[_FakeJob, Literal["created", "existed"]]] = []
        seen_steps: set[str] = set()
        member_count = sum(followup.workflow_member is True for followup in followups or ())
        workflow = (
            self._workflows[job.command.workflow_id]
            if job.command.workflow_id is not None
            else None
        )
        if member_count:
            if workflow is None or job.continuation_policy_hash is None:
                raise TaskqValidationError(details={"reason": "continuation_parent_not_member"})
        for followup in followups or ():
            ...
            if queue not in self._known_queues:
                raise TaskqNotFoundError()
            key = derive_continuation_idempotency_key(job.job_id, followup.step)
            identity = (queue, key)
            existing_id = self._active_keys.get(identity)
            member_step = (
                derive_continuation_workflow_step(job.job_id, followup.step)
                if followup.workflow_member is True
                else None
            )
            command = EnqueueCommand(
                queue=queue,
                ...
                # first, then install the already-derived private identity.
                step_key=("internal" if member_step is not None else None),
            )
            if member_step is not None:
                command = command.model_copy(update={"step_key": member_step})
            if (
                followup.workflow_member is True
                and workflow is not None
                and queue not in workflow.declared_queues
            ):
                raise TaskqValidationError(details={"reason": "continuation_queue_undeclared"})
            if existing_id is None:
                child = _FakeJob(
                    ...
                    continuation_policy_hash=(
                        job.continuation_policy_hash if followup.workflow_member is True else None
                    ),
                )
        ...
        new_member_count = sum(
            status == "created" and child.command.workflow_id is not None
            for child, status in children
        )
        if new_member_count:
            assert workflow is not None and workflow.steps is not None
            if (
                workflow.member_limit is None
                or workflow.admitted_total + new_member_count > workflow.member_limit
            ):
                raise TaskqConflictError(details={"reason": "workflow_member_limit_exceeded"})
        self._finish(
            job,
        ...
        for child, status in children:
            if status == "created":
                ...
                if child.command.workflow_id is not None:
                    assert workflow is not None and workflow.steps is not None
                    assert child.command.step_key is not None
                    workflow.steps[child.command.step_key] = child.job_id
                    workflow.admitted_total += 1
            self._enqueues.append(
                RecordedEnqueue(
                    job_id=child.job_id,
    ...
    async def worker_heartbeat(
        ...
        *,
        ...
        self._ensure_open()
        try:
            request = WorkerPresenceWireRequest(
                worker_id=worker_id,
                queues=tuple(queues),
                hostname=hostname,
                pid=pid,
            ...
            raise TaskqValidationError(details={"field": str(exc.errors()[0]["loc"][0])}) from exc
        now = self._now()
        existing = self._workers.get(worker_id)
        self._workers[worker_id] = _FakeWorkerPresence(
            worker_id=worker_id,
            declared_queues=request.queues,
            version=request.version,
            started_at=existing.started_at if existing is not None else now,
            last_seen_at=now,
            shutdown_requested=(existing.shutdown_requested if existing is not None else False),
        )
        return False
    ...
    async def list_worker_presence(
        ...
        *,
        ...
        if (after_last_seen_at is None) != (after_worker_id is None):
            raise TaskqValidationError(details={"field": "cursor"})
        now = self._now()
        ordered = sorted(
            self._workers.values(),
            key=lambda item: (item.last_seen_at, item.worker_id),
            reverse=True,
        )
        if after_last_seen_at is not None and after_worker_id is not None:
            ordered = [
                item
                for item in ordered
                if (item.last_seen_at, item.worker_id) < (after_last_seen_at, after_worker_id)
            ]
        selected = ordered[: limit + 1]
        next_item = selected[limit - 1] if len(selected) > limit else None
        selected = selected[:limit]
        items = tuple(
            WorkerPresenceItem(
                worker_id=item.worker_id,
                declared_queues=item.declared_queues,
                version=item.version,
                started_at=item.started_at,
                last_seen_at=item.last_seen_at,
                online=item.last_seen_at > now - timedelta(seconds=180),
                running_jobs=sum(
                    1
                    for job in self._jobs.values()
                    if job.worker_id == item.worker_id and job.status is JobStatus.RUNNING
                ),
                shutdown_requested=item.shutdown_requested,
            )
        ...
        return WorkerPresencePage(
            as_of=now,
            items=items,
            next_last_seen_at=next_item.last_seen_at if next_item is not None else None,
            next_worker_id=next_item.worker_id if next_item is not None else None,
        )
