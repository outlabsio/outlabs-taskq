...
class Followup(BaseModel):
    ...
    workflow_member: Literal[True] | None = None

    @field_validator("workflow_member", mode="before")
    @classmethod
    def _workflow_member_is_true_or_null(cls, value: object) -> object:
        if value is not None and value is not True:
            raise ValueError("workflow_member may only be true when present")
        return value
...
class ClaimWireRequest(BaseModel):
    ...
    @classmethod
    def _supported_policy_hashes(cls, value: tuple[str, ...] | None) -> tuple[str, ...] | None:
        if value is None:
            return None
        if len(set(value)) != len(value) or any(
            ...
            raise ValueError(
                "supported_policy_hashes must contain distinct lowercase SHA-256 values"
            )
...
class WorkerPresenceWireRequest(BaseModel):
    ...
    worker_id: str = Field(min_length=1, max_length=200)
    queues: tuple[str, ...] = Field(min_length=1, max_length=32)
    ...
    def _distinct_queues(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        ...
        if any(not re.fullmatch(r"[a-z0-9_]{1,57}", queue) for queue in value):
            raise ValueError("queues contains an invalid queue name")
        return value
...
class WorkerPresenceItem(BaseModel):
    model_config = ConfigDict(frozen=True, extra="ignore")
...
class WorkerPresencePage(BaseModel):
    model_config = ConfigDict(frozen=True, extra="ignore")

    as_of: datetime
    items: tuple[WorkerPresenceItem, ...]
    next_last_seen_at: datetime | None = None
    next_worker_id: str | None = Field(default=None, min_length=1, max_length=200)

    @model_validator(mode="after")
    def _cursor_pair(self) -> WorkerPresencePage:
        if (self.next_last_seen_at is None) != (self.next_worker_id is None):
            raise ValueError("worker presence cursor fields must be both present or both absent")
        return self


class WorkerPresencePageWireData(BaseModel):
    model_config = ConfigDict(frozen=True, extra="ignore")
...
class WorkflowReadProfile(BaseModel):
    ...
    admitted_total: int | None = Field(default=None, ge=0, le=1_000_000)
    remaining_capacity: int | None = Field(default=None, ge=0, le=1_000_000)
    continuation_policy_hash: str | None = Field(default=None, pattern=r"^[0-9a-f]{64}$")

    @model_validator(mode="after")
    def _continuation_capacity_is_coherent(self) -> WorkflowReadProfile:
        capacity = (self.member_limit, self.admitted_total, self.remaining_capacity)
        if all(value is None for value in capacity):
            return self
        if any(value is None for value in capacity):
            raise ValueError(
                "workflow continuation capacity fields must be all present or all null"
            )
        if self.admitted_total > self.member_limit:  # type: ignore[operator]
            raise ValueError("admitted_total cannot exceed member_limit")
        if self.remaining_capacity != self.member_limit - self.admitted_total:  # type: ignore[operator]
            raise ValueError("remaining_capacity must equal member_limit minus admitted_total")
        return self
...
HTTP_COMMAND_SPECS: Final = MappingProxyType(
    {
        ...
        HttpCommandName.LIST_WORKERS: _http(
            ...
            _status_map(ok=200),
            _SAFE,
            CommandName.LIST_WORKER_PRESENCE,
            data_model=WorkerPresencePageWireData,
        ),
