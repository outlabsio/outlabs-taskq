...
class TaskqFacadeTransports:
    ...
    worker_presence_enabled: bool = False
...
def _decode_worker_presence_cursor(value: str | None) -> tuple[datetime, str] | None:
    if value is None:
        return None
    if len(value) > 1366 or not re.fullmatch(r"[A-Za-z0-9_-]+", value):
        raise TaskqValidationError(details={"field": "cursor"})
    ...
    if (
        len(raw) > 1024
        or not isinstance(decoded, dict)
        or set(decoded) != {"last_seen_at", "v", "worker_id"}
        or decoded.get("v") != 1
        or not isinstance(decoded.get("worker_id"), str)
        or not 1 <= len(decoded["worker_id"]) <= 200
        or not isinstance(decoded.get("last_seen_at"), str)
    ):
        raise TaskqValidationError(details={"field": "cursor"})
    try:
    ...
    if last_seen_at.tzinfo is None:
        raise TaskqValidationError(details={"field": "cursor"})
    return last_seen_at, decoded["worker_id"]


def _encode_worker_presence_cursor(
    last_seen_at: datetime | None, worker_id: str | None
) -> str | None:
    if last_seen_at is None and worker_id is None:
        return None
    if last_seen_at is None or worker_id is None:
        raise TaskqInternalError(details={"reason": "invalid_worker_presence_cursor_pair"})
    raw = json.dumps(
        {"last_seen_at": last_seen_at.isoformat(), "v": 1, "worker_id": worker_id},
        sort_keys=True,
        separators=(",", ":"),
    ).encode()
    return base64.urlsafe_b64encode(raw).decode().rstrip("=")
...
class _FacadeDispatcher:
        ...
        child_queues = sorted(
            {
                ...
                if followup.queue is not None and followup.queue != parent_queue
            }
        )
    ...
    async def _execute(
        ...
        if name is HttpCommandName.CREATE_WORKFLOW:
            ...
            if body.member_limit is not None:
                continuation["member_limit"] = body.member_limit
            if body.continuation_policy_hash is not None:
                continuation["continuation_policy_hash"] = body.continuation_policy_hash
        ...
        if name is HttpCommandName.LIST_WORKERS:
            if not r.worker_presence_enabled:
                raise TaskqCapabilityError(details={"capability": "worker_presence"})
            limit = _parse_page_limit(request.query_params.get("limit"))
            after = _decode_worker_presence_cursor(request.query_params.get("cursor"))
            page = await r.observer.list_worker_presence(
                limit=limit,
                after_last_seen_at=after[0] if after is not None else None,
                after_worker_id=after[1] if after is not None else None,
            )
            return _command_response(
                request,
                spec,
                "ok",
                {
                    "as_of": page.as_of,
                    "items": page.items,
                    "next_cursor": _encode_worker_presence_cursor(
                        page.next_last_seen_at, page.next_worker_id
                    ),
                },
            )
