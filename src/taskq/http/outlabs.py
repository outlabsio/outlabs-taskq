...
class _OutlabsPrincipal:
    ...
    authenticated_result: dict[str, Any] = field(repr=False, compare=False)
    request_scope: Mapping[str, Any] = field(repr=False, compare=False)
...
class OutlabsQueueAuthorizer:
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(getattr(deps, "authorize_authenticated", None)) or not callable(
            getattr(deps, "authenticated_authorization_requires_session", None)
        ):
            raise TypeError("OutLabs auth must expose its authenticated-context authorization API")

    def __repr__(self) -> str:
        return "OutlabsQueueAuthorizer(" + (
            f"authenticated_context=True, legacy_actions={tuple(sorted(self._extra_candidates))})"
        )
    ...
    def _context(self, request: Request, result: dict[str, Any]) -> AuthContext:
        copied = MappingProxyType(dict(result))
        actor = _bounded_actor(self._actor_from_principal(copied))
        return AuthContext(
            actor=actor,
            principal=_OutlabsPrincipal(
                copied,
                _principal_fingerprint(copied),
                result,
                request.scope,
            ),
        )
    ...
    async def authenticate(self, request: Request) -> AuthContext:
        ...
        return self._context(request, result)
    ...
    async def authorize_context(
        ...
        if context.principal.request_scope is not request.scope:
            raise HTTPException(status_code=401, detail="authentication changed during request")

        deps = self._auth.deps
        names = self._names(action, queue)
        auth_result = context.principal.authenticated_result
        try:
            if deps.authenticated_authorization_requires_session(auth_result):
                async with _resolved_session(self._session_dependency, request) as session:
                    result = await deps.authorize_authenticated(
                        request,
                        auth_result,
                        *names,
                        require_all=False,
