...
WORKFLOW_READ_SQL_CONTRACT_VERSIONS = frozenset(
    {"0.2.3", "0.2.4", "0.2.5", "0.2.6", "0.2.7", "0.3.0", "0.3.1", "0.4.0"}
)
SCHEDULE_SQL_CONTRACT_VERSIONS = frozenset({"0.3.0", "0.3.1", "0.4.0"})
    ...
    def from_dsn(
        ...
        *,
        ...
        facade = TaskqFacadeTransports(
            ...
            worker_presence_enabled=resolved.worker_presence_enabled,
        )
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            if self.facade_transports.workflow_read_enabled:
                if meta.contract_version not in WORKFLOW_READ_SQL_CONTRACT_VERSIONS:
                    raise TaskqVersionError(details={"contract_version": meta.contract_version})
                active = meta.capabilities.get("active")
                required = {
            ...
            if self.facade_transports.worker_presence_enabled:
                if meta.contract_version not in WORKER_PRESENCE_SQL_CONTRACT_VERSIONS:
                    raise TaskqVersionError(details={"contract_version": meta.contract_version})
                active = meta.capabilities.get("active")
                if not isinstance(active, list) or "worker_presence" not in active:
